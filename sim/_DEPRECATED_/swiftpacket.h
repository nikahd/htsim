...
public:
    ...
    inline static SwiftPacket* newpkt(
        PacketFlow& flow, const Route& route, seq_t seqno, seq_t dsn, int size) {
        SwiftPacket* p = _packetdb.allocPacket();
        p->set_route(flow,
                     route,
                     size,
                     seqno + size - 1);  // The Swift sequence number is the first byte of the
    ...
    inline static SwiftPacket* new_syn_pkt(PacketFlow&  flow,
                                           const Route& route,
                                           seq_t        seqno,
                                           int          size) {
    ...
    }  // change this if you want to use swift with priority queues
...
public:
    ...
    inline static SwiftAck* newpkt(PacketFlow&     flow,
                                   const Route&    route,
                                   seq_t           seqno,
                                   seq_t           ackno,
                                   seq_t           ds_ackno,
