...
void DCTCPSrc::receivePacket(Packet& pkt) {
    ...
    if (_pkts_seen * _mss >= _past_cwnd) {
        ...
        //        cout << ntoa(timeAsMs(eventlist().now())) << " ID " << str() << " PKTS MARKED " <<
        //        _pkts_marks;
        ...
        // cout << ntoa(timeAsMs(eventlist().now())) << " UPDATE " << str() << " CWND " << _cwnd <<
        // " alfa " << ntoa(_alfa)<< " marked " << ntoa(f) << endl;
    }

    TcpSrc::receivePacket(pkt);
    // cout << ntoa(timeAsMs(eventlist().now())) << " ATCPID " << str() << " CWND " << _cwnd << "
    // alfa " << ntoa(_alfa)<< endl;
}
