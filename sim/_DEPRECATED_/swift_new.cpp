...
SwiftSubflowSrc::SwiftSubflowSrc(SwiftSrc& src, TrafficLogger* pktlogger)
    ...
    // swift cc init
    _swift_cwnd = 12 * src._mss;  // initial window, in bytes  Note: values in paper are in packets;
...
void SwiftSubflowSrc::adjust_cwnd(simtime_picosec delay, SwiftAck::seq_t ackno) {
    ...
    if (_src.plb()) {
        simtime_picosec td =
            _src.targetDelay(false, *_route);  // target delay ignoring cwnd component
...
void SwiftSubflowSrc::send_packets() {
...
void SwiftSubflowSrc::rtx_timer_hook(simtime_picosec now, simtime_picosec period) {
    ...
    cout << timeAsUs(eventlist().now()) << " " << _src.nodename() << " At "
         << now / (double)1000000000 << " RTO " << _rto / 1000000000 << " MDEV "
         << _mdev / 1000000000 << " RTT " << _rtt / 1000000000 << " SEQ " << _last_acked / mss()
         << " HSENT " << _highest_sent << " CWND " << _swift_cwnd / mss() << " FAST RECOVERY? "
...
void SwiftSubflowSrc::move_path() {
    ...
    assert(_path_index <
           _src._paths
               .size());  // if we've moved paths so often we've run out of paths, I want to know
...
SwiftSrc::SwiftSrc(SwiftLogger* logger, TrafficLogger* pktlogger, EventList& eventlst)
    ...
    cout << eventlist().now() << " " << nodename() << " created, cwnd " << _sub->_swift_cwnd
         << endl;
}

void SwiftSrc::log(SwiftLogger::SwiftEvent event) {
    if (_logger)
...
void SwiftSrc::set_cwnd(uint32_t cwnd) {
    ...
    cout << eventlist().now() << " " << nodename() << " set_cwnd, cwnd " << _sub->_swift_cwnd
         << endl;
}
...
void SwiftSrc::startflow() {
    ...
    cout << eventlist().now() << " " << nodename() << " started, cwnd " << _sub->_swift_cwnd
         << endl;
...
simtime_picosec SwiftSrc::targetDelay(bool add_fs_delay, const Route& route) {
    // note fs_delay can be negative, so don't use simtime_picosec here!
    double fs_delay =
        _fs_alpha / sqrt(_sub->_swift_cwnd / _mss) + _fs_beta;  // _sub->_swift_cwnd is in bytes
...
void SwiftSrc::receivePacket(Packet& pkt) {
    ...
    // _inflate << endl; cout << timeAsUs(eventlist().now()) << " " << nodename() << " highest_sent
    // was  " << _sub->_highest_sent << endl;
...
void SwiftSink::send_ack(simtime_picosec ts) {
    ...
    SwiftAck* ack =
        SwiftAck::newpkt(_src->_sub->flow(), *rt, 0, _cumulative_ack, _cumulative_data_ack, ts);
...
void SwiftRtxTimerScanner::registerSwift(SwiftSrc& src) {
