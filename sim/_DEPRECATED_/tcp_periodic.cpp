...
TcpSrcPeriodic::TcpSrcPeriodic(TcpLogger*      logger,
                               TrafficLogger*  pktLogger,
                               EventList&      eventlist,
                               simtime_picosec active,
...
void TcpSrcPeriodic::connect(const Route&    routeout,
...
void TcpSrcPeriodic::doNextEvent() {
    ...
    if (_is_active) {
        if (eventlist().now() >= _end_active && _idle_time != 0 && _active_time != 0) {
            ...
            cout << "Wrong place to be in: doNextDEvent 1" << eventlist().now() << " end active "
                 << _end_active << "timeout " << _rtx_timeout_pending << endl;
...
void TcpSrcPeriodic::receivePacket(Packet& pkt) {
    if (_is_active)
