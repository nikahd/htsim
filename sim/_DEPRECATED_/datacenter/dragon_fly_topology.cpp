...
DragonFlyTopology::DragonFlyTopology(uint32_t        p,
                                     uint32_t        a,
                                     uint32_t        h,
                                     mem_b           queuesize,
                                     Logfile*        lg,
    ...
    cout << "DragonFly topology with " << _p << " hosts per router, " << _a
         << " routers per group and " << (_a * _h + 1) << " groups, total nodes " << _no_of_nodes
         << endl;
...
DragonFlyTopology::DragonFlyTopology(uint32_t        no_of_nodes,
                                     mem_b           queuesize,
                                     Logfile*        lg,
                                     EventList*      ev,
                                     queue_type      q,
...
Queue* DragonFlyTopology::alloc_src_queue(QueueLogger* queueLogger) {
    return new FairPriorityQueue(
        speedFromMbps((uint64_t)HOST_NIC), memFromPkt(FEEDER_BUFFER), *_eventlist, queueLogger);
    // return new PriorityQueue(speedFromMbps((uint64_t)HOST_NIC), memFromPkt(FEEDER_BUFFER),
...
Queue* DragonFlyTopology::alloc_queue(QueueLogger* queueLogger,
                                      uint64_t     speed,
                                      mem_b        queuesize,
                                      bool         tor) {
    if (qt == RANDOM)
        return new RandomQueue(
            speedFromMbps(speed), queuesize, *_eventlist, queueLogger, memFromPkt(RANDOM_BUFFER));
        ...
        return new ECNQueue(
            speedFromMbps(speed), memFromPkt(queuesize), *_eventlist, queueLogger, memFromPkt(15));
    else if (qt == LOSSLESS)
        return new LosslessQueue(
            speedFromMbps(speed), memFromPkt(50), *_eventlist, queueLogger, NULL);
    else if (qt == LOSSLESS_INPUT)
        return new LosslessOutputQueue(
            speedFromMbps(speed), memFromPkt(200), *_eventlist, queueLogger);
    else if (qt == LOSSLESS_INPUT_ECN)
        return new LosslessOutputQueue(
            speedFromMbps(speed), memFromPkt(10000), *_eventlist, queueLogger);
    else if (qt == COMPOSITE_ECN) {
        if (tor)
            return new CompositeQueue(
                speedFromMbps(speed), queuesize, *_eventlist, queueLogger, 64);
        else
            return new ECNQueue(speedFromMbps(speed),
                                memFromPkt(2 * SWITCH_BUFFER),
                                *_eventlist,
                                queueLogger,
                                memFromPkt(15));
    }
...
void DragonFlyTopology::init_network() {
    ...
    for (uint32_t j = 0; j < _no_of_switches; j++) {
        ...
        // Connect the switch to other switches in the same group, with higher IDs (full mesh within
        // group).
        ...
        for (uint32_t l = 0; l < _h; l++) {
            ...
            // if the ID is larger than ours, effective target group is +1 (we skip our own group
            // for global links).
            if (larger)
                targetgroupid++;
            else
...
vector<const Route*>* DragonFlyTopology::get_bidir_paths(uint32_t src,
                                                         uint32_t dest,
                                                         bool     reverse) {
    ...
    if (HOST_TOR(src) == HOST_TOR(dest)) {
        ...
        if (qt == LOSSLESS_INPUT || qt == LOSSLESS_INPUT_ECN)
            routeout->push_back(
                queues_switch_switch[HOST_TOR(src)][HOST_TOR(dest)]->getRemoteEndpoint());
        ...
        if (qt == LOSSLESS_INPUT || qt == LOSSLESS_INPUT_ECN)
            routeback->push_back(
                queues_switch_switch[HOST_TOR(dest)][HOST_TOR(src)]->getRemoteEndpoint());
        ...
        // find srcswitch from srcgroup which has a path to dstgroup and  dstswitch from dstgroup
        // which has an incoming path from srcgroup.
        ...
        if (HOST_TOR(src) != srcswitch) {
            /*When SRC HOST TOR does not have a direct path to destination group, take local path to
            ...
            if (qt == LOSSLESS_INPUT || qt == LOSSLESS_INPUT_ECN)
                routeout->push_back(
                    queues_switch_switch[HOST_TOR(src)][srcswitch]->getRemoteEndpoint());
        }
        ...
        if (dstswitch != HOST_TOR(dest)) {
            /*When dstswitch does not have a direct path to dest, take local path to the appropriate
             * TOR switch*/
            ...
            if (qt == LOSSLESS_INPUT || qt == LOSSLESS_INPUT_ECN)
                routeout->push_back(
                    queues_switch_switch[dstswitch][HOST_TOR(dest)]->getRemoteEndpoint());
        }
        // cout << "DEST " << dest <<        " " << endl;
        ...
        */
        ...
        for (uint32_t p = 0; p < _no_of_groups; p++) {
            ...
            // cout << "DPSRC " << src << " SW " << HOST_TOR(src) << " " <<
            ...
            // find srcswitch from srcgroup which has a path to intergroup and dstswitch from
            // intergroup which has an incoming path from srcgroup.
            ...
            if (HOST_TOR(src) != srcswitch) {
                /*When SRC HOST TOR does not have a direct path to destination group, take local
                 * path to the appropriate switch*/
            ...
            if (qt == LOSSLESS_INPUT || qt == LOSSLESS_INPUT_ECN)
                routeout->push_back(
                    queues_host_switch[srcswitch][interswitch1]->getRemoteEndpoint());
            ...
            if (interswitch1 != interswitch2) {
                /*When interswitch1 does not have a direct path to destination group, take local
                 * path to the appropriate switch*/
                ...
                if (qt == LOSSLESS_INPUT || qt == LOSSLESS_INPUT_ECN)
                    routeout->push_back(
                        queues_switch_switch[interswitch1][interswitch2]->getRemoteEndpoint());
            }
            ...
            if (qt == LOSSLESS_INPUT || qt == LOSSLESS_INPUT_ECN)
                routeout->push_back(
                    queues_host_switch[interswitch2][dstswitch]->getRemoteEndpoint());

            if (dstswitch != HOST_TOR(dest)) {
                /*When dstswitch does not have a direct path to dest, take local path to the
                 * appropriate TOR switch*/
                // cout << "SW " << HOST_TOR(dest) <<        " ";
                assert(queues_switch_switch[dstswitch][HOST_TOR(dest)]);
