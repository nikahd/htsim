...
void VL2Topology::init_network() {
    ...
    for (uint32_t j = 0; j < NT; j++) {
        for (uint32_t k = 0; k < NS; k++) {
            ...
            queues_nt_ns[j][k] = new RandomQueue(speedFromPktps(HOST_NIC),
            ...
            queueLogger = new QueueLoggerSampling(timeFromMs(1000), *eventlist);
            logfile->addLogger(*queueLogger);
            queues_ns_nt[k][j] = new RandomQueue(speedFromPktps(HOST_NIC),
                                                 memFromPkt(SWITCH_BUFFER + RANDOM_BUFFER),
                                                 *eventlist,
                                                 queueLogger,
                                                 memFromPkt(RANDOM_BUFFER));
    ...
    for (uint32_t j = 0; j < NT; j++) {
        ...
        for (uint32_t l = 0; l < NT2A; l++) {
            ...
            queueLogger = new QueueLoggerSampling(timeFromMs(1000), *eventlist);
            logfile->addLogger(*queueLogger);
            queues_na_nt[k][j] = new RandomQueue(speedFromPktps(CORE_TO_HOST * HOST_NIC),
                                                 memFromPkt(SWITCH_BUFFER + RANDOM_BUFFER),
                                                 *eventlist,
                                                 queueLogger,
                                                 memFromPkt(RANDOM_BUFFER));
            ...
            queueLogger = new QueueLoggerSampling(timeFromMs(1000), *eventlist);
            logfile->addLogger(*queueLogger);
            queues_nt_na[j][k] = new RandomQueue(speedFromPktps(CORE_TO_HOST * HOST_NIC),
                                                 memFromPkt(SWITCH_BUFFER + RANDOM_BUFFER),
                                                 *eventlist,
                                                 queueLogger,
                                                 memFromPkt(RANDOM_BUFFER));
    ...
    for (uint32_t j = 0; j < NI; j++) {
        for (uint32_t k = 0; k < NA; k++) {
            ...
            queues_ni_na[j][k] = new RandomQueue(speedFromPktps(CORE_TO_HOST * HOST_NIC),
                                                 memFromPkt(SWITCH_BUFFER + RANDOM_BUFFER),
                                                 *eventlist,
                                                 queueLogger,
                                                 memFromPkt(RANDOM_BUFFER));
            ...
            queues_na_ni[k][j] = new RandomQueue(speedFromPktps(CORE_TO_HOST * HOST_NIC),
                                                 memFromPkt(SWITCH_BUFFER + RANDOM_BUFFER),
                                                 *eventlist,
                                                 queueLogger,
                                                 memFromPkt(RANDOM_BUFFER));
...
vector<const Route*>* VL2Topology::get_paths(uint32_t src, uint32_t dest) {
    ...
    if (HOST_TOR(src) == HOST_TOR(dest)) {
        Queue* pqueue = new Queue(
            speedFromPktps(CORE_TO_HOST * HOST_NIC), memFromPkt(FEEDER_BUFFER), *eventlist, NULL);
