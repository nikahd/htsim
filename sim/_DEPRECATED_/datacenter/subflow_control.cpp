...
SubflowControl::SubflowControl(simtime_picosec         scanPeriod,
                               Logfile*                lg,
                               SinkLoggerSampling*     sl,
                               EventList&              eventlist,
...
void SubflowControl::run() {
    ...
    for (it = flow_counters.begin(); it != flow_counters.end(); it++) {
        ...
        //    cout << "Delta " << delta << "subs " <<f->subflows->size() << "max " << _max_subflows
        //    << "counts " << counts << endl ;
        if (counts && delta < threshold && f->subflows->size() < _max_subflows &&
            ...
            tcpSrc->setName("mtcp_" + ntoa(f->src) + "_" + ntoa(f->subflows->size()) + "_" +
                            ntoa(f->dest));
            logfile->writeName(*tcpSrc);

            tcpSnk->setName("mtcp_sink_" + ntoa(f->src) + "_" + ntoa(f->subflows->size()) + "_" +
                            ntoa(f->dest));
            ...
            // cout << endl << ntoa(f->subflows->size()) << " subflows between " << ntoa(f->src) <<
            // " and " << ntoa(f->dest) << " at " << timeAsMs(eventlist().now()) << endl ;
            ...
            double extrastarttime =
                timeAsMs(eventlist().now()) + timeAsMs(_scanPeriod) * drand() + 1;
...
void SubflowControl::print_stats() {
    ...
    for (it = flow_counters.begin(); it != flow_counters.end(); it++) {
        ...
        cout << endl
             << ntoa(f->subflows->size()) << " subflows between " << ntoa(f->src) << " and "
             << ntoa(f->dest) << endl;
    }
}
