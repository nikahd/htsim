...
Queue* OversubscribedFatTreeTopology::alloc_queue(QueueLogger* queueLogger, uint64_t speed) {
    if (qt == RANDOM)
        return new RandomQueue(
            speedFromMbps(speed), _queuesize, *eventlist, queueLogger, memFromPkt(RANDOM_BUFFER));
    else if (qt == COMPOSITE)
        return new CompositeQueue(
            speedFromMbps(speed), _queuesize, *eventlist, queueLogger, FatTreeSwitch::_trim_size);
    else if (qt == ECN)
        return new ECNQueue(
            speedFromMbps(speed), _queuesize, *eventlist, queueLogger, memFromPkt(15));
    assert(0);
}
...
vector<const Route*>* OversubscribedFatTreeTopology::get_bidir_paths(uint32_t src,
                                                                     uint32_t dest,
                                                                     bool     reverse) {
