...
#ifdef FAT_TREE
    FatTreeTopology* top =
        new FatTreeTopology(no_of_nodes, linkspeed, memFromPkt(8), &qlf, &eventlist, ff, RANDOM, 0);
#endif
...

            {
                ...
                for (uint32_t connection = 0; connection < 1; connection++) {
                    ...
                    for (uint32_t inter = 0; inter < it_sub; inter++) {
                        ...
                        tcpSnk->setName("mtcp_sink_" + ntoa(src) + "_" + ntoa(inter) + "_" +
                        ...
                        if (inter == 0) {
                            mtcp->setName("multipath" + ntoa(src) + "_" + ntoa(dest) + "(" +
                                          ntoa(connection) + ")");
