...
int main(int argc, char** argv) {
    ...
    FatTreeTopology* top = new FatTreeTopology(no_of_nodes, linkspeed, queuesize,
                                               &logfile, &eventlist, NULL, RANDOM, SWIFT_SCHEDULER,
    0);
    */
    FatTreeTopology* top = new FatTreeTopology(
        no_of_nodes, linkspeed, queuesize, NULL, &eventlist, NULL, RANDOM, SWIFT_SCHEDULER, 0);
#endif
    ...
    for (uint32_t c = 0; c < all_conns->size(); c++) {
        ...
        // ReorderBufferLoggerSampling* buf_logger = new
        // ReorderBufferLoggerSampling(timeFromMs(0.01), eventlist); logfile.addLogger(*buf_logger);
        ...
        if (no_of_subflows > 1) {
            ...
            swiftSrc->multipath_connect(
                *swiftSnk, timeFromUs((uint32_t)crt->start), no_of_subflows);
        }
    ...
    for (src_i = swift_srcs.begin(); src_i != swift_srcs.end(); src_i++) {
        cout << "Src, sent: " << (*src_i)->_packets_sent << "[new: " << (*src_i)->_new_packets_sent
    << " rtx: " << (*src_i)->_rtx_packets_sent << "] nacks: " << (*src_i)->_nacks_received << "
    pulls: " << (*src_i)->_pulls_received << " paths: " << (*src_i)->_paths.size() << endl;
    }
    for (src_i = swift_srcs.begin(); src_i != swift_srcs.end(); src_i++) {
        (*src_i)->print_stats();
        }*/
