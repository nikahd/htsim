...
bool GenericTopology::load(const char* filename) {
    ...
    // we need to do two passes, one to build the ID tables, and one to fill in all the
    // cross-references
...
bool get_attribute(std::vector<std::string>& tokens,
                   uint32_t&                 ix,
...
Host* GenericTopology::find_host(const string& id) {
    // linear scan though vector is slow, but we only do this at startup so probably OK.  Otherwise
    // add a map.
...
void GenericTopology::parse_queue(std::vector<std::string>& tokens, int pass, std::fstream& gv) {
    ...
    if (pass == 0) {
        ...
        q = find_queue(id);
        if (!q)
            cerr << "failed to find queue " << id << endl;
        assert(q);
    }
    // On pass 0, we will create the queue, once we know what type of queue to create, and the
    // relevant params On pass 1, we will link it into the network
    vector<string> attribute;
    string         queuetype;
    ...
    if (pass == 0) {
        ...
        if (queuetype == "queue") {
            ...
            q = new RandomQueue(
                linkspeed, queuesize, *_eventlist, queuelogger, memFromPkt(RANDOM_BUFFER));
