...
BCubeTopology::BCubeTopology(uint32_t        no_of_nodes,
                             uint32_t        ports_per_switch,
...
void BCubeTopology::set_params(uint32_t no_of_nodes,
                               uint32_t ports_per_switch,
                               uint32_t no_of_levels) {
...
Queue* BCubeTopology::alloc_src_queue(QueueLogger* queueLogger) {
    return new PriorityQueue(
        speedFromMbps((uint64_t)HOST_NIC), memFromPkt(FEEDER_BUFFER), *eventlist, queueLogger);
}
...
Queue* BCubeTopology::alloc_queue(QueueLogger* queueLogger, uint64_t speed) {
    if (qt == RANDOM) {
        ...
        return new CompositeQueue(speedFromMbps(speed), memFromPkt(8), *eventlist, queueLogger, 64);
    } else if (qt == COMPOSITE_PRIO) {
        return new CompositePrioQueue(
            speedFromMbps(speed), memFromPkt(8), *eventlist, queueLogger, 64);
    }
...
void BCubeTopology::init_network() {
    ...
    for (uint32_t k = 0; k <= _K; k++) {
        ...
        for (uint32_t i = 0; i < _NUM_SRV; i++) {
            ...
            queues_srv_switch(i, j, k)->setName("SRV_" + ntoa(i) + "(level_" + ntoa(k) + "))_SW_" +
                                                ntoa(j));
            logfile->writeName(*(queues_srv_switch(i, j, k)));

            pipes_srv_switch(i, j, k) = new Pipe(_rtt, *eventlist);
            pipes_srv_switch(i, j, k)->setName("Pipe-SRV_" + ntoa(i) + "(level_" + ntoa(k) +
                                               ")-SW_" + ntoa(j));
            ...
            queues_switch_srv(j, i, k)->setName("SW_" + ntoa(j) + "(level_" + ntoa(k) + ")-SRV_" +
                                                ntoa(i));
            logfile->writeName(*(queues_switch_srv(j, i, k)));

            pipes_switch_srv(j, i, k) = new Pipe(_rtt, *eventlist);
            pipes_switch_srv(j, i, k)->setName("Pipe-SW_" + ntoa(j) + "(level_" + ntoa(k) +
                                               ")-SRV_" + ntoa(i));
...
Route* BCubeTopology::bcube_routing(uint32_t  src,
                                    uint32_t  dest,
