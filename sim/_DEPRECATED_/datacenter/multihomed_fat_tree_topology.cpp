...
MultihomedFatTreeTopology::MultihomedFatTreeTopology(Logfile*        lg,
                                                     EventList*      ev,
                                                     FirstFit*       fit,
                                                     simtime_picosec rtt) {
    logfile   = lg;
...
void MultihomedFatTreeTopology::init_network() {
    ...
    for (uint32_t j = 0; j < NLP; j++) {
        for (uint32_t l = 0; l < 2 * K / 3; l++) {
            ...
            queues_nlp_ns[j][k] = new RandomQueue(speedFromPktps(HOST_NIC),
    ...
    for (uint32_t j = 0; j < NLP; j++) {
        ...
        for (uint32_t k = start; k <= end; k++) {
            ...
            queueLogger = new QueueLoggerSampling(timeFromMs(1000), *eventlist);
            logfile->addLogger(*queueLogger);
            queues_nup_nlp[k][j] = new RandomQueue(speedFromPktps(HOST_NIC),
                                                   memFromPkt(SWITCH_BUFFER + RANDOM_BUFFER),
                                                   *eventlist,
                                                   queueLogger,
                                                   memFromPkt(RANDOM_BUFFER));
            ...
            queueLogger = new QueueLoggerSampling(timeFromMs(1000), *eventlist);
            logfile->addLogger(*queueLogger);
            queues_nlp_nup[j][k] = new RandomQueue(speedFromPktps(HOST_NIC),
                                                   memFromPkt(SWITCH_BUFFER + RANDOM_BUFFER),
                                                   *eventlist,
                                                   queueLogger,
                                                   memFromPkt(RANDOM_BUFFER));
    ...
    for (uint32_t j = 0; j < NK; j++) {
        ...
        for (uint32_t l = 0; l < K / 2; l++) {
            ...
            queues_nup_nc[j][k] = new RandomQueue(speedFromPktps(HOST_NIC),
                                                  memFromPkt(SWITCH_BUFFER + RANDOM_BUFFER),
                                                  *eventlist,
                                                  queueLogger,
                                                  memFromPkt(RANDOM_BUFFER));
            ...
            //        if (k==0&&j==0)
            // queues_nc_nup[k][j] = new RandomQueue(speedFromPktps(HOST_NIC/10),
            // memFromPkt(SWITCH_BUFFER + RANDOM_BUFFER), *eventlist, queueLogger,
            // memFromPkt(RANDOM_BUFFER)); else
            queues_nc_nup[k][j] = new RandomQueue(speedFromPktps(HOST_NIC),
                                                  memFromPkt(SWITCH_BUFFER + RANDOM_BUFFER),
                                                  *eventlist,
                                                  queueLogger,
                                                  memFromPkt(RANDOM_BUFFER));
...
vector<const Route*>* MultihomedFatTreeTopology::get_paths(uint32_t src, uint32_t dest) {
    ...
    if (HOST_POD_SWITCH1(src) == HOST_POD_SWITCH1(dest)) {
        Queue* pqueue =
            new Queue(speedFromPktps(2 * HOST_NIC), memFromPkt(FEEDER_BUFFER), *eventlist, NULL);
        ...
        for (uint32_t upper = MIN_POD_ID(pod); upper <= MAX_POD_ID(pod); upper++) {
            // upper is nup
            Queue* pqueue =
                new Queue(speedFromPktps(HOST_NIC), memFromPkt(FEEDER_BUFFER), *eventlist, NULL);
            pqueue->setName("PQueue_" + ntoa(src) + "_" + ntoa(dest));
            // logfile->writeName(*pqueue);
        ...
        for (uint32_t upper = MIN_POD_ID(pod); upper <= MAX_POD_ID(pod); upper++)
            for (uint32_t core = (upper % (K / 2)) * K / 2; core < ((upper % (K / 2)) + 1) * K / 2;
                 core++) {
                // upper is nup
                Queue* pqueue = new Queue(
                    speedFromPktps(HOST_NIC), memFromPkt(FEEDER_BUFFER), *eventlist, NULL);
                pqueue->setName("PQueue_" + ntoa(src) + "_" + ntoa(dest));
                // logfile->writeName(*pqueue);
                ...
                if (upper2 <= (MIN_POD_ID(pod_dest) + MAX_POD_ID(pod_dest)) / 2) {
                    //          cout << "Upper 2S "<<upper2 << " MIN_POD " << MIN_POD_ID(pod_dest)<<
                    //          " MAX_POD " << MAX_POD_ID(pod_dest)<<endl;
                    swd = HOST_POD_SWITCH1(dest);
                } else {
                    // cout << "Upper 2B "<<upper2 << " MIN_POD " << MIN_POD_ID(pod_dest)<< "
                    // MAX_POD " << MAX_POD_ID(pod_dest)<<endl;
                    swd = HOST_POD_SWITCH2(dest);
                }
