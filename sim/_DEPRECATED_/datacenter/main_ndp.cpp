...
void exit_error(char* progr) {
    cout << "Usage " << progr
         << " [-nodes N]\n\t[-conns C]\n\t[-cwnd cwnd_size]\n\t[-q "
            "queue_size]\n\t[-oversubscribed_cc] Use receiver-driven AIMD to reduce total window "
            ...
            "wire latency in us,default 1\n\t[-switch_latency x] switching latency in us, default "
...
void filter_paths(uint32_t src_id, vector<const Route*>& paths, FatTreeTopology* top) {
    ...
    cout << "srv: " << num_servers << " cores: " << num_cores << " pods: " << num_pods
         << " pod_sw: " << pod_switches << " classes: " << path_classes << endl;
    ...
    switch (route_strategy) {
        ...
        case SINGLE_PATH:
            if (path_entropy_size < 10000 && path_entropy_size > 1) {
                fprintf(stderr,
                        "Route Strategy is SINGLE_PATH, but multiple paths are specifiec using "
                        "-paths\n");
                exit(1);
            }
            break;
    ...
    for (size_t c = 0; c < all_conns->size(); c++) {
        ...
        switch (route_strategy) {
            ...
            case REACTIVE_ECN: {
                ...
                srctotor->push_back(
                    top->queues_ns_nlp[src][top->HOST_POD_SWITCH(src)][0]->getRemoteEndpoint());
                ...
                dsttotor->push_back(
                    top->queues_ns_nlp[dest][top->HOST_POD_SWITCH(dest)][0]->getRemoteEndpoint());
