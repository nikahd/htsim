...
SwiftSubflowSrc::SwiftSubflowSrc(SwiftSrc& src, TrafficLogger* pktlogger, int sub_id)
    : EventSource(src.eventlist(), "swift_subflow_src"),
      _flow(pktlogger),
      _src(src),
    ...
    // swift cc init
    _swift_cwnd = _default_cwnd;  // initial window, in bytes  Note: values in paper are in packets;
                                  // we're maintaining in bytes.
    _retransmit_cnt = 0;
    _can_decrease   = true;
...
void SwiftSubflowSrc::adjust_cwnd(simtime_picosec delay, SwiftAck::seq_t ackno) {
    ...
    _can_decrease =
        (now - _last_decrease) >= _rtt;  // not clear if we should use smoothed RTT here.
    // can_decrease = true;

    // compute rtt
    update_rtt(delay);

    // Swift cwnd calculation.  Doing this here does it for every ack, no matter if we're in fast
    // recovery or not.  Need to be careful.
    _retransmit_cnt              = 0;
    simtime_picosec target_delay = _src.targetDelay(_swift_cwnd, *_route);
    // cout << timeAsUs(now) << " delay (us)" << timeAsUs(delay) << " target " <<
    // timeAsUs(target_delay) << endl;
    if (delay < target_delay) {
        // cout << "ackno " << ackno << " cwnd " << _swift_cwnd << " inf " <<  _inflate << "
        // last_acked " <<  _last_acked << endl;
    ...
    if (_src.plb()) {
        ...
        if (now - _last_good_path > _plb_interval) {
            cout << "moving " << timeAsUs(now) << " last_good " << timeAsUs(_last_good_path)
                 << " RTT " << timeAsUs(_rtt) << endl;
            _last_good_path = now;
            move_path();
        }
    }
...
void SwiftSubflowSrc::applySwiftLimits() {
    // we call this whenever we've changed cwnd to enforce bounds, just before we actually use the
    // cwnd
    ...
    if (_swift_cwnd < mss()) {
        ...
        // cout << "swift_cwnd " << ((double)_sub->_swift_cwnd)/_mss << " pacing " <<
...
void SwiftSubflowSrc::handle_ack(SwiftAck::seq_t ackno) {
    ...
    if (ackno > _last_acked) {              // a brand new ack
        ...
        if (ackno >= _highest_sent) {
            _highest_sent = ackno;
            // cout << timeAsUs(now) << " " << nodename() << " highest_sent now  " << _highest_sent
            // << endl;
        ...
        if (ackno >= _recoverq) {
            ...
            //  in NewReno, we'd reset the cwnd here, but I think in swift we continue with the
            //  delay-adjusted cwnd.
...
int SwiftSubflowSrc::send_packets() {
    ...
    while ((_last_acked + c >= _highest_sent + mss()) && _src.more_data_available()) {
        ...
        if (_RFC2988_RTO_timeout == timeInf) {  // RFC2988 5.1
            _RFC2988_RTO_timeout = eventlist().now() + _rto;
            // cout << timeAsUs(eventlist().now()) << " " << nodename() << " RTO at " <<
            // timeAsUs(_RFC2988_RTO_timeout) << "us" << endl;
        }
    }
    return sent_count;
}
...
bool SwiftSubflowSrc::send_next_packet() {
    ...
    // cout << timeAsUs(eventlist().now()) << " " << nodename() << " sent " << _highest_sent+1 <<
    // "-" << _highest_sent+mss() << " dsn " << dsn << endl;
    _highest_sent += mss();
    _packets_sent += mss();
...
void SwiftSubflowSrc::receivePacket(Packet& pkt) {
    ...
    // cout << timeAsUs(eventlist().now()) << " " << nodename() << " recvack  " << ackno << "
    // ts_echo " << timeAsUs(ts_echo) << endl; cout << timeAsUs(eventlist().now()) << " " <<
    // nodename() << " highest_sent was  " << _sub->_highest_sent << endl;
...
void SwiftSubflowSrc::rtx_timer_hook(simtime_picosec now, simtime_picosec period) {
    ...
    cout << timeAsUs(eventlist().now()) << " " << nodename() << " At " << now / (double)1000000000
         << " RTO " << _rto / 1000000000 << " MDEV " << _mdev / 1000000000 << " RTT "
         << _rtt / 1000000000 << " SEQ " << _last_acked / mss() << " HSENT " << _highest_sent
         << " CWND " << _swift_cwnd / mss() << " FAST RECOVERY? " << _in_fast_recovery
         << " Flow ID " << str() << endl;
...
void SwiftSubflowSrc::connect(SwiftSink&     sink,
                              const Route&   routeout,
                              const Route&   routeback,
                              uint32_t       flow_id,
                              BaseScheduler* scheduler) {
    _subflow_sink    = sink.connect(_src, *this, routeback);
    Route* new_route = routeout.clone();  // make a copy, as we may be switching routes and don't
                                          // want to append the sink more than once
...
EventList& SwiftSubflowSrc::eventlist() const {
    return _src.eventlist();
}

uint16_t SwiftSubflowSrc::mss() const {
    return _src.mss();
}

double SwiftSubflowSrc::ai() const {
    return _src.ai();
}

double SwiftSubflowSrc::beta() const {
    return _src.beta();
}

double SwiftSubflowSrc::max_mdf() const {
    return _src.max_mdf();
}
...
SwiftSrc::SwiftSrc(SwiftRtxTimerScanner& rtx_scanner,
                   SwiftLogger*          logger,
                   TrafficLogger*        pktlogger,
                   EventList&            eventlst)
    : EventSource(eventlst, "swift"),
      _logger(logger),
    ...
    _h = _base_delay /
         6.55;  // path length scaling constant.  Value is a guess, will be clarified by experiment
    _rtx_reset_threshold = 5;  // value is a guess
    _min_cwnd = 10;            // guess - if we go less than 10 bytes, we probably get into rounding
    _max_cwnd = 1000 * _mss;   // maximum cwnd we can use.  Guess - how high should we allow cwnd to
                               // go?  Presumably something like B*target_delay?
...
void SwiftSrc::set_cwnd(uint32_t cwnd) {
    for (size_t i = 0; i < _subs.size(); i++) {
        ...
        // cout << eventlist().now() << " " << _subs[i]->nodename() << " set_cwnd, cwnd " <<
        // _subs[i]->_swift_cwnd << endl;
    }
...
void SwiftSrc::set_hdiv(double hdiv) {
    _h = _base_delay /
         hdiv;  // path length scaling constant.  Value is a guess, will be clarified by experiment
}
...
void SwiftSrc::set_paths(vector<const Route*>* rt_list) {
    ...
    for (size_t i = 0; i < no_of_paths; i++) {
        ...
        if (!_scheduler) {
            ...
            // sanity check all paths share the same scheduler.  If we ever want to use multiple
            // NICs, this will need fixing
...
void SwiftSrc::startflow() {
    for (size_t i = 0; i < _subs.size(); i++) {
        ...
        // cout << eventlist().now() << " " << _subs[i]->nodename() << " (sub" << i << ") started,
        // cwnd " << _subs[i]->_swift_cwnd << endl;
        _subs[i]->send_packets();
    }
}
...
void SwiftSrc::connect(const Route&    routeout,
                       const Route&    routeback,
                       SwiftSink&      sink,
...
void SwiftSrc::multipath_connect(SwiftSink&      sink,
                                 simtime_picosec starttime,
...
simtime_picosec SwiftSrc::targetDelay(uint32_t cwnd, const Route& route) {
    ...
    // cout << "fs_delay " << fs_delay << " range " << _fs_range << " _sub->_swift_cwnd/_mss " <<
    // _sub->_swift_cwnd/_mss << " sqrt " << sqrt(_sub->_swift_cwnd/_mss) << " beta " << _fs_beta <<
    // endl; cout << "fs_alpha " << _fs_alpha << endl;
...
void SwiftSrc::update_dsn_ack(SwiftAck::seq_t ds_ackno) {
    ...
    if (ds_ackno >= _flow_size) {
        cout << "Flow " << _name << " finished at " << timeAsUs(eventlist().now())
             << " total bytes " << ds_ackno << endl;
    }
}

uint32_t SwiftSrc::drops() {
    int drops = 0;
    for (size_t i = 0; i < _subs.size(); i++) {
...
void SwiftSubflowSink::send_ack(simtime_picosec ts) {
    ...
    SwiftAck* ack = SwiftAck::newpkt(
        _subflow_src->flow(), *rt, 0, _cumulative_ack, _sink._cumulative_data_ack, ts);

    ack->flow().logTraffic(*ack, *this, TrafficLogger::PKT_CREATESEND);
    ack->sendOn();
}

const string& SwiftSubflowSink::nodename() {
    return _sink.nodename();
}

uint64_t SwiftSubflowSink::cumulative_ack() {
...
uint32_t SwiftSubflowSink::drops() {
    if (_sink._src) {
...
uint64_t SwiftSink::cumulative_ack() {
    // this is needed by some loggers.  If we ever need it, figure out what it should really return
    return _src ? _cumulative_data_ack + _src->_mss : 0;
}

uint32_t SwiftSink::drops() {
