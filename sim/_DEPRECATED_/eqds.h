...
public:
    ...
    EqdsSrc(TrafficLogger* trafficLogger,
            EventList&     eventList,
            EqdsNIC&       nic,
    ...
    virtual void connectPort(
        uint32_t portnum, Route& routeout, Route& routeback, EqdsSink& sink, simtime_picosec start);
...
private:
    ...
    mem_b _credit_spec;
    mem_b _pull_offset;  // remember if we've artificially inflated pull target to avoid negative
                         // values
