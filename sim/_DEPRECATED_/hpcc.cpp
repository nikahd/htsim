...
HPCCSrc::HPCCSrc(HPCCLogger*    logger,
    ...
    cout << "Initial CWND is " << _cwnd << " target RTT " << timeAsUs(_T) << " rate " << rate
         << endl;
    _flightsize = 0;
    _U          = _eta;
}
...
void HPCCSrc::connect(Route*          routeout,
                      Route*          routeback,
                      HPCCSink&       sink,
...
void HPCCSrc::processNack(const HPCCNack& nack) {
    ...
    cout << "HPCC " << _name << " go back n from " << _highest_sent << " to " << _last_acked
...
void HPCCSrc::processAck(const HPCCAck& ack) {
    ...
    */
    if (ackno > _last_update_seq) {
        ...
        cout << "CWND1 " << _cwnd << " ACKNO " << ackno << " at " << timeAsUs(eventlist().now())
             << " src " << _nodename << endl;
        _last_update_seq = _highest_sent;
    } else {
        _cwnd = computeWind(measureInFlight(ack), false);
        cout << "CWND2 " << _cwnd << " ACKNO " << ackno << " at " << timeAsUs(eventlist().now())
             << " src " << _nodename << endl;
    }
    ...
    // cout << "INT Entries " << ack._int_hop << " qs " << ack._int_info[0]._queuesize << " TS " <<
    // timeAsUs(ack._int_info[0]._ts) << " TX " << ack._int_info[0]._txbytes << endl;
...
void HPCCSrc::send_packet() {
    ...
    if (_flow_size && _highest_sent + _mss >= _flow_size) {
        last_packet = true;
        // cout << _name << " sending last packet with SEQNO " << _highest_sent+1 << " at " <<
...
void HPCCSink::receivePacket(Packet& pkt) {
    ...
    if (seqno > _cumulative_ack + 1) {
        ...
        // cout << "Wrong seqno received at HPCC SINK " << seqno << " expecting " << _cumulative_ack
        // << endl;
        return;
    }
