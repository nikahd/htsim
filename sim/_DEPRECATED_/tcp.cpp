...
void TcpSrc::connect(const Route&    routeout,
...
void TcpSrc::receivePacket(
    Packet& pkt) {  // cerr << eventlist().now() / 1e12 << " " << __PRETTY_FUNCTION__ << ":(" <<
                    // int(pkt.dst()) << "," << int(pkt.flow_id()) << ")" << endl;
    simtime_picosec ts;
    TcpAck*         p     = (TcpAck*)(&pkt);
    ...
    //  cout << "Base "<<timeAsMs(_base_rtt)<< " RTT " << timeAsMs(_rtt)<< " Queued " <<
    //  queued_packets << endl;
    ...
    if (seqno > _last_acked) {  // a brand new ack
        ...
        //   cout << "ACK "<<cnt<<" packets on " << _flow.id << " " << _highest_sent+1 << " packets
        //   in flight " << _sent_packets.crt_count << " diff " <<
        //   (_highest_sent+_mss-_last_acked)/1000 << " last_acked " << _last_acked << " at " <<
        //   timeAsMs(eventlist().now()) << endl;
#endif

        if (!_in_fast_recovery) {  // best behaviour: proper ack of a new packet, when we were
                                   // expecting it
            // clear timers
    ...
    if (_cwnd < _ssthresh) {  // slow start
        ...
        if (_mSrc == NULL) {
            ...
            _cwnd += (newly_acked * _mss) /
                     _cwnd;  // XXX beware large windows, when this increase gets to be very small
...
void TcpSrc::send_packets() {
    ...
    while ((_last_acked + c >= _highest_sent + _mss) && (_highest_sent <= _flow_size + 1)) {
        ...
        if (data_seq > _highest_data_seq)
            ...
            //      cout << "Transmit packet on " << _flow.id << " " << _highest_sent+1 << "[" <<
            //      data_seq << "] packets in flight " << _sent_packets.crt_count << " diff " <<
            //      (_highest_sent+_mss-_last_acked)/1000 << " last_acked " << _last_acked << " at "
            //      << timeAsMs(eventlist().now()) << endl;
#endif
        ...
        if (_paths) {
            ...
            p = TcpPacket::newpkt(
                _flow, *(_paths->at(_crt_path)), _highest_sent + 1, data_seq, _mss);
...
void TcpSrc::retransmit_packet() {
    ...
    if (!_sent_packets.get_data_seq(_last_acked + 1, &data_seq)) {
        cout << "Failed to find packet on " << _flow.id << " last_acked " << _last_acked + 1
             << " highest_sent " << _highest_sent << endl;
        assert(NULL);
    }
    //  else
    //  cout << "Retransmit packet on " << _flow.id << " " << _last_acked+1 << " " << data_seq <<
    //  endl;
#endif
...
void TcpSrc::rtx_timer_hook(simtime_picosec now, simtime_picosec period) {
    ...
    cout << "At " << now / (double)1000000000 << " RTO " << _rto / 1000000000 << " MDEV "
         << _mdev / 1000000000 << " RTT " << _rtt / 1000000000 << " SEQ " << _last_acked / _mss
         << " HSENT " << _highest_sent << " CWND " << _cwnd / _mss << " FAST RECOVERY? "
...
void TcpSink::receivePacket(Packet& pkt) {
    // cerr << _src->eventlist().now()/1e12 << " " << __PRETTY_FUNCTION__  << ":(" << int(pkt.dst())
    // << "," << int(pkt.flow_id()) << ")" << endl;
...
void TcpSink::send_ack(simtime_picosec ts, bool marked) {
    ...
    TcpAck* ack = TcpAck::newpkt(
        _src->_flow, *rt, 0, _cumulative_ack, _mSink != NULL ? _mSink->data_ack() : 0);
