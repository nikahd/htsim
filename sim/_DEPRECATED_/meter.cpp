...
Meter::Meter(linkspeed_bps bitrate, mem_b maxsize, EventList& eventlist)
    ...
    stringstream ss;
    ss << "meter(" << bitrate / 1000000 << "Mb/s," << maxsize << "bytes), psperbyte "
       << _ps_per_byte;

    _nodename = ss.str();
}

void Meter::beginService() {
    /* schedule the next dequeue event */
    assert(!_enqueued.empty());
    eventlist().sourceIsPendingRel(*this, drainTime(_enqueued.back()));
}

void Meter::completeService() {
    /* dequeue the packet */
    assert(!_enqueued.empty());
...
void Meter::doNextEvent() {
    completeService();
}

meter_output Meter::execute(Packet& pkt) {
    ...
    // cout << "Adding credit " << delta/service_rate << "total " << _credit << "delta " << delta <<
    // " service rate " << service_rate << " Times " << eventlist().now() << " " << _endpesitrim <<
    // " " << _endsemipesitrim << endl;
...
mem_b Meter::queuesize() {
    return _queuesize;
}

simtime_picosec Meter::serviceTime() {
