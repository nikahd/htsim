...
NdpSrc::NdpSrc(NdpLogger*     logger,
               TrafficLogger* pktlogger,
               EventList&     eventlist,
               bool           rts,
               NdpRTSPacer*   rts_pacer)
...
void NdpSrc::startflow() {
    cout << "startflow " << _flow._name << " CWND " << _cwnd << " rts " << _rts << " at "
...
bool NdpSrc::is_bad_path() {
    ...
    if (ack_count > 0 && total / ack_count <= 3) {
        printf(
            "total: %d ack: %d nack:%d rts: %d, BAD\n", total, ack_count, nack_count, bounce_count);
        return true;
    }
    // printf("total: %d ack: %d nack:%d rts: %d, NOT BAD\n", total, ack_count, nack_count,
    // bounce_count);
    return false;
}
...
void NdpSrc::processNack(const NdpNack& nack) {
    ...
    */
    ...
    switch (_route_strategy) {
        ...
        case ECMP_FIB_ECN:
            p = NdpPacket::newpkt(
                _flow, *_route, seqno, 0, _mss, true, _path_ids.size(), last_packet, _dstaddr);
    ...
    switch (pkt.type()) {
        ...
        case NDPPULL: {
            ...
            */
            ...
            /*
            if (_log_me)
                cout << "Received PULL at " << timeAsUs(eventlist().now()) << " pullno " <<
            p->pullno() << endl;
            */
...
int NdpSrc::choose_route() {
    switch (_route_strategy) {
        ...
        case SCATTER_PERMUTE:
        case SCATTER_ECMP:
            // Cycle through a permutation.  Generally gets better load balancing than
            // SCATTER_RANDOM.
            _crt_path++;
            assert(_paths.size() > 0);
        ...
        case ECMP_FIB_ECN: {
            ...
            while (1) {
                ...
                if (_path_ecns[_path_ids[_crt_path]] > 0) {
                    ...
                    /*
                    if (_log_me) {
                        cout << eventlist().now() << " skipped " << _path_ids[_crt_path] << " " <<
                    _path_ecns[_path_ids[_crt_path]] << endl;
                    }
                    */
                } else {
...
void NdpSrc::pull_packets(NdpPull::seq_t pull_no, NdpPull::seq_t pacer_no) {
    ...
    // cout << "Last pull " << _last_pull << " pull no " << pull_no << " max pull " << _max_pull <<
    // endl;
    if (pull_no > _max_pull)
        _max_pull = pull_no;
    while (_last_pull < _max_pull) {
        int sent = send_packet(pacer_no);
        // cout << "Sending packet out, sent=" << sent << " last=" << _last_pull << " max=" <<
        // _max_pull << endl;
        _last_pull += sent;
        if (sent == 0) {
            break;
...
void NdpSrc::retransmit_packet() {
    ...
    while (i != _sent_times.end()) {
        if (i->second + _rto <= eventlist().now()) {
            // cout << "_sent_time: " << timeAsUs(i->second) << "us rto " << timeAsUs(_rto) << "us
            // now " << timeAsUs(eventlist().now()) << "us\n"; this one is due for retransmission
    ...
    for (j = rtx_list.begin(); j != rtx_list.end(); j++) {
        ...
        switch (_route_strategy) {
            ...
            case SCATTER_ECMP: {
                ...
                p               = NdpPacket::newpkt(
                    _flow, *rt, seqno, 0, _mss, true, _paths.size(), last_packet, _dstaddr);
            ...
            case SINGLE_PATH:
                p = NdpPacket::newpkt(
                    _flow, *_route, seqno, 0, _mss, true, _paths.size(), last_packet, _dstaddr);
                break;
            case NOT_SET:
                abort();
        }
...
void NdpSrc::rtx_timer_hook(simtime_picosec now, simtime_picosec period) {
    ...
    cout << "At " << timeAsUs(now) << "us RTO " << timeAsUs(_rto) << "us MDEV " << timeAsUs(_mdev)
         << "us RTT " << timeAsUs(_rtt) << "us SEQ " << _last_acked / _mss << " CWND "
         << _cwnd / _mss << " Flow ID " << str() << endl;
...
void NdpSink::receiver_core_trim(NdpPacket* p) {
    ...
    cout << "ReduceRate " << _src->_nodename << " Parked " << _parked_cwnd << " Increase at "
         << _parked_increase << endl;
}
...
void NdpSink::receiver_ecn_accounting(NdpPacket* p) {
    ...
    if (p->pacerno() >= _ecn_decrease) {
        ...
        cout << "Alpha set to " << _alpha << " M " << M << " marked " << _marked_bytes << " acked "
             << _acked_bytes << endl;
...
void NdpSink::receiver_increase(NdpPacket* p) {
    ...
    if (p->pacerno() >= _parked_increase && _parked_cwnd > (uint32_t)Packet::data_packet_size()) {
        cout << "IssuePull " << _src->_nodename << " Parked " << _parked_cwnd << " Increase at "
             << _parked_increase << " packet pacer " << p->pacerno() << endl;
...
void NdpSink::receivePacket(Packet& pkt) {
    switch (pkt.type()) {
        ...
        case NDPNACK:
        case NDPPULL:
            // Is there anything we should do here?  Generally won't happen unless the topolgy is
            // very asymmetric.
            assert(pkt.bounced());
    ...
    if (pkt.header_only()) {
        ...
        // cout << "Header seqno " << seqno << " highest " << _highest_seqno << " total " <<
        // _total_received << " flow " << this << " at " << timeAsMs(_pacer->eventlist().now()) <<
        // endl;
#ifdef RECORD_PATH_LENS
    ...
    int size =
        p->size() -
        NdpPacket::ACKSIZE;  // TODO: the following code assumes all packets are the same size
    ...
    if (ooo_distance <
        (int)(seqno -
              _cumulative_ack)) {  // careful - we want a signed comparison - RHS would be unsigned
    ...
    if (seqno == _cumulative_ack + 1) {  // it's the next expected seq no
        ...
        if (_received.empty()) {
            ...
            // commenting out the code below, probably copied from TCP and innacurate for NDP where
            // reordering is expected it's a drop in this simulator there are no reorderings. _drops
            //+= (size + seqno-_cumulative_ack-1)/size;
...
void NdpSink::send_ack(simtime_picosec  ts,
                       NdpPacket::seq_t ackno,
...
void NdpSink::send_nack(simtime_picosec  ts,
                        NdpPacket::seq_t ackno,
                        NdpPacket::seq_t pacer_no,
                        bool             enqueue_pull,
                        bool             ecn_marked) {
    ...
    switch (_route_strategy) {
        ...
        case SCATTER_ECMP:
            assert(_paths.size() > 0);
            nack = NdpNack::newpkt(_src->_flow,
                                   *(_paths.at(_crt_path)),
                                   0,
                                   ackno,
                                   _cumulative_ack,
        ...
        case REACTIVE_ECN:
            ...
            // set ECN echo only if that is selected strategy
            nack->set_ecn_echo(
                ecn_marked && (_route_strategy == ECMP_FIB_ECN || _route_strategy == REACTIVE_ECN));
            break;
        case SINGLE_PATH:
            nack = NdpNack::newpkt(_src->_flow,
                                   *_route,
...
NdpPullPacer::NdpPullPacer(EventList& event, linkspeed_bps linkspeed, double pull_rate_modifier)
    ...
    _packet_drain_time = (simtime_picosec)((Packet::data_packet_size() + NdpPacket::ACKSIZE) *
    ...
    if (_pull_queue.empty()) {
        ...
        if (delta >= drain_time) {
            ...
            if (_log_me) {
                ...
                if (ack->type() == NDPACK) {
                    cout << "Ack " <<  (((NdpAck*)ack)->ackno()-1)/9000 << " excess " << excess << "
            (no queue)\n"; } else if (ack->type() == NDPNACK) { cout << "Nack " <<
            (((NdpNack*)ack)->ackno()-1)/9000 << " excess " << excess << " (no queue)\n"; } else {
                    cout << "WTF\n";
                }
                }
            */
...
void NdpPullPacer::enqueue_pull(NdpPull* pkt, NdpSink* receiver) {
    ...
    /*
    if (_log_me){
        cout << "Enqueue pull, pull queue size is " << _pull_queue.pull_count() << " should_tx is "
    << should_tx << " delta " << timeAsUs(delta) << " drain " << timeAsUs(_packet_drain_time) <<
    endl;
    }
    */
...
void NdpRTSPacer::enqueue_rts(NdpRTS* pkt) {
    ...
    // cout << "Enqueue RTS, RTS queue size is " << _rts_queue.pull_count() << " should_tx is " <<
    // should_tx << " delta " << timeAsUs(delta) << " drain " << timeAsUs(_packet_drain_time) <<
    // endl;
