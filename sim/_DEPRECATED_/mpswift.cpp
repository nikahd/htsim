...
SwiftSrc::SwiftSrc(SwiftLogger* logger, TrafficLogger* pktlogger, EventList& eventlst)
    ...
    _swift_cwnd = 12 * _mss;  // initial window, in bytes  Note: values in paper are in packets;
...
void SwiftSrc::move_path() {
    ...
    _path_index++;
    assert(_path_index <
           _paths.size());  // if we've moved paths so often we've run out of paths, I want to know
    _route = _paths[1];
}
...
simtime_picosec SwiftSrc::targetDelay(bool add_fs_delay) {
    ...
    // cout << "fs_delay " << fs_delay << " range " << _fs_range << " _swift_cwnd/_mss " <<
    // _swift_cwnd/_mss << " sqrt " << sqrt(_swift_cwnd/_mss) << " beta " << _fs_beta << endl; cout
    // << "fs_alpha " << _fs_alpha << endl;
...
void SwiftSrc::receivePacket(Packet& pkt) {
    ...
    if (ackno > _last_acked) {                            // a brand new ack
        ...
        if (ackno >= _highest_sent) {
            _highest_sent = ackno;
            // cout << timeAsUs(eventlist().now()) << " " << nodename() << " highest_sent now  " <<
            // _highest_sent << endl;
...
void SwiftSrc::rtx_timer_hook(simtime_picosec now, simtime_picosec period) {
    ...
    cout << timeAsUs(eventlist().now()) << " " << nodename() << " At " << now / (double)1000000000
         ...
         << _rtt / 1000000000 << " SEQ " << _last_acked / _mss << " HSENT " << _highest_sent
