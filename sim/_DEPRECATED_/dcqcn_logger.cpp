...
void DCQCNTrafficLogger::logTraffic(Packet& pkt, Logged& location, TrafficEvent ev) {
    ...
    _logfile->writeRecord(
        Logger::DCQCN_TRAFFIC, location.get_id(), ev, p.flow().get_id(), p.id(), val3);
}
...
string DCQCNTrafficLogger::event_to_str(RawLogEvent& event) {
    ...
    if (val3i & ROCE_IS_ACK) {
        ss << " Ptype ACK" << " Ackno " << (uint64_t)event._val2;
    } else if (val3i & ROCE_IS_NACK) {
        ss << " Ptype NACK" << " Ackno " << (uint64_t)event._val2;
    } else {
        ss << " Ptype DATA" << " Seqno " << (uint64_t)event._val2;
...
DCQCNSinkLoggerSampling::DCQCNSinkLoggerSampling(simtime_picosec period, EventList& eventlist)
...
string DCQCNSinkLoggerSampling::event_to_str(RawLogEvent& event) {
    ...
    switch (event._type) {
        case Logger::HPCC_SINK:
            ...
            ss << " Type DCQCN_SINK ID " << event._id << " Ev RATE" << " CAck "
               << (uint64_t)event._val1 << " Rate " << (uint64_t)event._val3;
