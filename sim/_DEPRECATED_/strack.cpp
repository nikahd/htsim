...
STrackSrc::STrackSrc(STrackRtxTimerScanner& rtx_scanner,
                     STrackLogger*          logger,
    ...
    // strack cc init
    _strack_cwnd = _default_cwnd;  // initial window, in bytes  Note: values in paper are in
                                   // packets; we're maintaining in bytes.
    _retransmit_cnt = 0;
    _can_decrease   = true;
...
void STrackSrc::adjust_cwnd(simtime_picosec delay, STrackAck::seq_t ackno) {
    ...
    // STrack cwnd calculation.  Doing this here does it for every ack, no matter if we're in fast
    ...
    if (delay < target_delay) {
        ...
        if (delay > target_delay * 2 && _achieved_BDP > 0) {
            ...
            _strack_cwnd =
                _strack_cwnd * max(1 - beta() * (delay - target_delay) / delay, 1 - max_mdf());
        }
        _last_decrease = now;
    }
}
...
void STrackSrc::applySTrackLimits() {
    ...
    if (_strack_cwnd < mss()) {
        ...
        // cout << "strack_cwnd " << ((double)_sub->_strack_cwnd)/_mss << " pacing " <<
...
void STrackSrc::handle_ack(STrackAck::seq_t ackno) {
    ...
    if (ackno > _last_acked) {              // a brand new ack
        ...
        if (ackno >= _recoverq) {
            ...
            //  in NewReno, we'd reset the cwnd here, but I think in strack we continue with the
            //  delay-adjusted cwnd.
...
int STrackSrc::send_packets() {
    ...
    int      sent_count = 0;
    // cout << eventlist().now() << " " << nodename() << " cwnd " << _strack_cwnd << " + " <<
    // _inflate << endl;
    if (!_established) {
...
void STrackSrc::rtx_timer_hook(simtime_picosec now, simtime_picosec period) {
    ...
    cout << timeAsUs(eventlist().now()) << " " << nodename() << " At " << now / (double)1000000000
         ...
         << " CWND " << _strack_cwnd / mss() << " FAST RECOVERY? " << _in_fast_recovery
         << " Flow ID " << str() << endl;
...
void STrackSrc::connect(const Route&    routeout,
...
simtime_picosec STrackSrc::targetDelay(const Route& route) {
    /*
    ...
    //cout << "fs_delay " << fs_delay << " range " << _fs_range << " _sub->_strack_cwnd/_mss " <<
    _sub->_strack_cwnd/_mss << " sqrt " << sqrt(_sub->_strack_cwnd/_mss) << " beta " << _fs_beta <<
    endl;
...
uint64_t STrackSink::cumulative_ack() {
