    ...
    if (_last_acked + _receive_window > _highest_sent) {
        *seq    = _highest_sent + 1;
        ...
        if (slow_subflow_id >= 0 && slow_subflow_id < 4 &&
            ...
            if ((src->effective_window() / timeAsMs(src->_rtt)) <
                    subflow->effective_window() / timeAsMs(subflow->_rtt) &&
        ...
        while (packet < _highest_sent) {
            ...
            // if (subflow->_highest_data_seq<packet ||
            // !subflow->_sent_packets.has_data_seq(packet)){
...
uint32_t MultipathTcpSrc::inflate_window(uint32_t cwnd, int newly_acked, uint32_t mss) {
    ...
    switch (_cc_type) {
        ...
        case COUPLED_INC:
            ...
            //    cout << timeAsMs(eventlist().now()) << " ID " << id << " Tmp "<< tmp << " tmp2 "
            //    << tmp2 << " tt " << tmp_float << " tcp inc " << tcp_inc << " alfa " << a << endl;
        ...
        case COUPLED_EPSILON:
            total_cwnd = compute_total_window();
            tmp_float  = ((double)newly_acked * mss * _alfa * pow(_alfa * cwnd, 1 - _e)) /
                        pow(total_cwnd, 2 - _e);
...
uint32_t MultipathTcpSrc::compute_a_scaled() {
    ...
    uint32_t alpha =
        (uint32_t)(A_SCALE * (uint64_t)cwndSum * t / sum_denominator / sum_denominator);
...
double MultipathTcpSrc::compute_a() {
    ...
    if (flow0->_rtt_avg > timeFromMs(0) && flow1->_rtt_avg > timeFromMs(1))
        ...
        // cout << "R1 " << flow0->_rtt_avg/1000000000 << " R2 " << flow1->_rtt_avg/1000000000 <<
        // endl;
#else
    rdelta = (double)flow0->_rtt / flow1->_rtt;
...
void MultipathTcpSink::receivePacket(Packet& pkt) {
    ...
    // cout << "at " << timeAsMs(eventlist().now()) << " Received on " << p->flow().id << " " <<
    // p->seqno() << " " << p->data_seqno() << " waiting for " << _cumulative_ack+1 << endl;
