...
TcpSrcTransfer::TcpSrcTransfer(TcpLogger*            logger,
...
void TcpSrcTransfer::connect(const Route&    routeout,
...
void TcpSrcTransfer::doNextEvent() {
    if (!_is_active) {
        _is_active = true;
...
void TcpSrcTransfer::receivePacket(Packet& pkt) {
    if (_is_active) {
        ...
        if (_bytes_to_send > 0) {
            if (!_mSrc && _last_acked >= _bytes_to_send) {
                ...
                if (_last_acked >= _bytes_to_send / _mSrc->_subflows.size() &&
                    _mSrc->compute_total_bytes() >= _bytes_to_send) {
                    // log finish time
...
void TcpSrcTransfer::rtx_timer_hook(simtime_picosec now, simtime_picosec period) {
    ...
    cout << "Transfer timeout: active " << _is_active << " bytes to send " << _bytes_to_send
         << " sent " << _last_acked << " established? " << _established << " HSENT "
         << _highest_sent << endl;
