...
void NdpTunnelSrc::connect(Route&          routeout,
                           Route&          routeback,
                           NdpTunnelSink&  sink,
...
const Route* NdpTunnelSrc::choose_route() {
    ...
    switch (_route_strategy) {
        ...
        case SINGLE_PATH:
            ...
            return _route;
        case PULL_BASED:  // added this to shut to compiler warnings - add real code if we ever hit
                          // this abort
            abort();
        case NOT_SET:
...
int NdpTunnelSrc::send_packet(NdpPull::seq_t pacer_no) {
    ...
    if (!_rtx_queue.empty()) {
        ...
        _qs -= pkt->size();
        // cout << "Sending packet into tunnel, qs " << _qs << " packet size " << pkt->size() << "
        // flightsize" << _flight_size << endl;
        ...
        switch (_route_strategy) {
            ...
            case PULL_BASED: {
                assert(_paths.size() > 0);
                const Route* rt = choose_route();
                p               = NdpTunnelPacket::newpkt(_flow,
                                            *rt,
                                            _highest_sent + 1,
                                            pacer_no,
                                            _mss,
            ...
            case SINGLE_PATH:
                p = NdpTunnelPacket::newpkt(
                    _flow, *_route, _highest_sent + 1, pacer_no, _mss, false, 1, false, pkt);
                break;
            case NOT_SET:
                abort();
        }
...
void NdpTunnelSink::receivePacket(Packet& pkt) {
    ...
    if (pkt.header_only()) {
        ...
        // not releasing this packet since it will be released by the sender once succesfully
        // rexmitted. p->free();
        return;
    }
    ...
    if (seqno == _cumulative_ack + 1) {  // it's the next expected seq no
        // cout << "Normal Delivery new PKT " << seqno << " ACK " << _cumulative_ack << " flow " <<
        // p->flow().flow_id() << endl;
        ...
        while (!_received.empty() && (_received.front()->seqno() == _cumulative_ack + 1)) {
            ...
            // cout << "Out of order delivery new PKT " << seqno << " ACK " << _cumulative_ack << "
            // flow " << p->flow().flow_id() << endl;
...
void NdpTunnelSink::send_ack(simtime_picosec        ts,
                             NdpTunnelPacket::seq_t ackno,
    ...
    switch (_route_strategy) {
        ...
        case PULL_BASED:
            assert(_paths.size() > 0);
            ack = NdpAck::newpkt(
                _src->_flow, *(_paths.at(_crt_path)), 0, ackno, _cumulative_ack, _pull_no, 0);
            if (_route_strategy == SCATTER_RANDOM) {
                _crt_path = random() % _paths.size();
            } else {
                _crt_path++;
...
void NdpTunnelSink::send_nack(simtime_picosec        ts,
                              NdpTunnelPacket::seq_t ackno,
                              NdpTunnelPacket::seq_t pacer_no) {
    NdpNack* nack = NULL;
    _pull_no++;
