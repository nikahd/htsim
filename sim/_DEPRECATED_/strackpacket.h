...
public:
    ...
    inline static STrackPacket* newpkt(PacketFlow&  flow,
                                       const Route& route,
                                       seq_t        seqno,
                                       int          size) {
        ...
        p->set_route(flow,
                     route,
                     size,
                     seqno + size - 1);  // The STrack sequence number is the first byte of the
    ...
    inline static STrackPacket* new_syn_pkt(PacketFlow&  flow,
                                            const Route& route,
                                            seq_t        seqno,
                                            int          size) {
...
public:
    ...
    inline static STrackAck* newpkt(
        PacketFlow& flow, const Route& route, seq_t seqno, seq_t ackno, simtime_picosec ts_echo) {
