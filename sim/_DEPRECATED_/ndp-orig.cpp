...
void NdpSrc::processNack(const NdpNack& nack) {
    ...
    */
    ...
    if (_route_strategy == SINGLE_PATH) {
        ...
        const Route* rt = choose_route();
        p               = NdpPacket::newpkt(
            _flow, *rt, seqno, 0, _mss, true, _paths.size() > 0 ? _paths.size() : 1, last_packet);
    }
    ...
    if (!_rtx_queue.empty()) {
        ...
        switch (_route_strategy) {
            ...
            case SCATTER_ECMP: {
                /*
                if (_log_me) {
                    cout << "Highest_sent: " << _highest_sent << " Flow_size: " << _flow_size <<
                endl;
                }
                */
                assert(_paths.size() > 0);
            ...
            case SINGLE_PATH:
                p = NdpPacket::newpkt(
                    _flow, *_route, _highest_sent + 1, pacer_no, _mss, false, 1, last_packet);
                break;
            case NOT_SET:
                abort();
        }
    ...
    for (j = rtx_list.begin(); j != rtx_list.end(); j++) {
        ...
        switch (_route_strategy) {
            ...
            case SCATTER_ECMP: {
                ...
                p = NdpPacket::newpkt(_flow, *rt, seqno, 0, _mss, true, _paths.size(), last_packet);
            ...
            case SINGLE_PATH:
                p = NdpPacket::newpkt(
                    _flow, *_route, seqno, 0, _mss, true, _paths.size(), last_packet);
                break;
            case NOT_SET:
                abort();
        }
...
void NdpSink::receivePacket(Packet& pkt) {
    ...
    int size =
        p->size() - ACKSIZE;  // TODO: the following code assumes all packets are the same size
        ...
        case SINGLE_PATH:
            ack = NdpAck::newpkt(_src->_flow,
                                 *_route,
                                 ...
                                 _cumulative_ack,
                                 _pull_no,
                                 0);  // _path_history[_path_hist_index].path_id());
            break;
        case NOT_SET:
            abort();
    }
...
NdpPullPacer::NdpPullPacer(EventList& event, linkspeed_bps linkspeed, double pull_rate_modifier)
    ...
    _packet_drain_time = (simtime_picosec)((Packet::data_packet_size() + ACKSIZE) *
...
void NdpPullPacer::enqueue_pull(NdpPull* pkt) {
    ...
    if (_log_me) {
        cout << "Enqueue pull, pull queue size is " << _pull_queue.pull_count() << " should_tx is "
             << should_tx << " delta " << timeAsUs(delta) << " drain "
