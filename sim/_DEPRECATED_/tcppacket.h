...
public:
    ...
    inline static TcpPacket* newpkt(
        PacketFlow& flow, const Route& route, seq_t seqno, seq_t dataseqno, int size) {
        TcpPacket* p = _packetdb.allocPacket();
        p->set_route(flow,
                     route,
                     size,
                     seqno + size - 1);  // The TCP sequence number is the first byte of the packet;
    ...
    inline static TcpPacket* new_syn_pkt(PacketFlow&  flow,
                                         const Route& route,
                                         seq_t        seqno,
                                         int          size) {
...
public:
    ...
    inline static TcpAck* newpkt(
        PacketFlow& flow, const Route& route, seq_t seqno, seq_t ackno, seq_t dackno) {
