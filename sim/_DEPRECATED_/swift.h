...
public:
    ...
    void schedule_send(simtime_picosec delay);  // schedule a paced packet "delay" picoseconds after
...
private:
    ...
    simtime_picosec
        _last_send;  // when the last packet was sent (always set, even when we're not pacing)
...
public:
    SwiftSrc(SwiftRtxTimerScanner& rtx_scanner,
             SwiftLogger*          logger,
             TrafficLogger*        pktlogger,
             EventList&            eventlist);
    void         log(SwiftSubflowSrc* sub, SwiftLogger::SwiftEvent event);
    virtual void connect(const Route&    routeout,
                         const Route&    routeback,
                         SwiftSink&      sink,
                         simtime_picosec startTime);
    virtual void multipath_connect(SwiftSink&      sink,
                                   simtime_picosec startTime,
...
public:
    ...
    set<SwiftPacket::seq_t>
        _dsn_received;  // use a map because multipath packets will arrive out of order
