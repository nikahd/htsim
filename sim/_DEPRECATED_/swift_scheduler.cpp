...
void BaseScheduler::receivePacket(Packet& pkt) {
    int flow_id = pkt.flow_id();
    // cout << "recv_packet " << this << " flow_id " << flow_id << " count " << _pkt_count << "
    // flow_count " << _queue_counts[flow_id] << endl;
    if (pkt.type() == SWIFT) {
...
void BaseScheduler::beginService() {
...
void BaseScheduler::doNextEvent() {
    completeService();
}

void BaseScheduler::completeService() {
...
Packet* FifoScheduler::next_packet() {
    return _queue.back();
}
...
void FairScheduler::enqueue(Packet& pkt) {
...
Packet* FairScheduler::next_packet() {
...
bool FairScheduler::queue_exists(const Packet& pkt) {
...
list<Packet*>* FairScheduler::find_queue(const Packet& pkt) {
    typename map<flowid_t, list<Packet*>*>::iterator i;
    i = _queue_map.find(pkt.flow_id());
    if (i == _queue_map.end())
        return 0;
