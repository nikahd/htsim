...
public:
    ...
    inline static NdpPacket* newpkt(PacketFlow& flow,
        ...
        p->set_attrs(flow,
                     size + ACKSIZE,
                     seqno + size - 1);  // The NDP sequence number is the first byte of the packet;
                                         // I will ID the packet by its last byte.
        p->_type          = NDP;
        p->_is_header     = false;
        p->_bounced       = false;
        p->_seqno         = seqno;
...
protected:
    seq_t _seqno;
    seq_t _pacerno;  // the pacer sequence number from the pull, seq space is common to all flows on
                     // that pacer
    simtime_picosec _ts;
...
public:
    ...
    inline static NdpAck* newpkt(PacketFlow&  flow,
                                 const Route& route,
                                 seq_t        pacerno,
                                 seq_t        ackno,
...
public:
    ...
    inline static NdpNack* newpkt(PacketFlow&  flow,
                                  const Route& route,
                                  seq_t        pacerno,
                                  seq_t        ackno,
                                  seq_t        cumulative_ack,
...
public:
    ...
    inline static NdpRTS* newpkt(PacketFlow&    flow,
                                 const route_t& route,
                                 int            grants,
                                 uint32_t       destination = UINT32_MAX) {
        NdpRTS* p = _packetdb.allocPacket();
...
public:
    ...
    inline static NdpPull* newpkt(NdpRTS* rts, seq_t cumack, seq_t pullno) {
        ...
        // cout << "Creating PULL packet with pull no " << p->_pullno << " received pull no " <<
        // pullno << endl;
        p->_path_len  = 0;
        p->_direction = NONE;
        p->set_dst(rts->dst());
    ...
    inline static NdpPull* newpkt(NdpRTS*        rts,
                                  const route_t& route,
                                  seq_t          cumack,
                                  seq_t          pullno,
                                  uint32_t       destination = UINT32_MAX) {
        ...
        // cout << "Creating PULL2 packet with pull no " << p->_pullno << " received pull no " <<
        // pullno << endl;
    ...
    inline static NdpPull* newpkt(PacketFlow&    flow,
                                  const route_t& route,
                                  seq_t          cumack,
                                  seq_t          pullno,
                                  uint32_t       destination = UINT32_MAX) {
