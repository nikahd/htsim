...
#define INIT_PULL \
    10000000  // needs to be large enough we don't map
...
//  EQDS SRC PORT
////////////////////////////////////////////////////////////////
EqdsSrcPort::EqdsSrcPort(EqdsSrc& src, uint32_t port_num) : _src(src), _port_num(port_num) {}
...
EqdsSrc::EqdsSrc(TrafficLogger* trafficLogger,
                 EventList&     eventList,
                 EqdsNIC&       nic,
...
void EqdsSrc::connectPort(uint32_t        port_num,
    ...
    if (port_num == 0) {
        _sink = &sink;
        //_flow.set_id(get_id());  // identify the packet flow with the EQDS source that generated
        // it
        _flow._name = _name;
...
bool EqdsSrc::checkFinished(EqdsDataPacket::seq_t cum_ack) {
    ...
    if (_debug_src)
        cout << _flow.str() << " " << _nodename << " checkFinished " << " cum_acc " << cum_ack
             << " mss " << _mss << " RTS sent " << _rts_packets_sent << " total bytes "
             << ((int64_t)cum_ack - _rts_packets_sent) * _mss << " flow_size " << _flow_size
...
void EqdsSrc::processAck(const EqdsAckPacket& pkt) {
    ...
    if (_debug_src)
        cout << _flow.str() << " " << _nodename << " processAck cum_ack: " << cum_ack << " flow "
             << _flow.str() << endl;
        ...
        if ((trimmed || need_quick_adapt) && previous_window_end != 0) {
            ...
            saved_acked_bytes =
                ...
                ((double)(_base_rtt + _target_Qdelay) /
                 (eventlist().now() - previous_window_end + (_base_rtt + _target_Qdelay)));
    ...
    if (skip == false && delay >= _target_Qdelay) {
        ...
        double delta1 =
            min(newly_acked_bytes,
                (mem_b)(_mi * _mtu * (target_rtt - (double)rtt) * newly_acked_bytes / _cwnd / rtt));
        _cwnd += delta1;
        ...
        if (can_decrease_exp_avg) {
            _cwnd -= min(newly_acked_bytes,
                         (mem_b)(_md * newly_acked_bytes * (rtt - (double)target_rtt) /
...
void EqdsSrc::processPull(const EqdsPullPacket& pkt) {
    ...
    auto pullno = pkt.pullno();
    if (_debug_src)
        cout << _flow.str() << " " << _nodename << " processPull " << pullno << " flow "
             << _flow.str() << " SP " << pkt.is_slow_pull() << endl;
...
EqdsBasePacket::pull_quanta EqdsSrc::computePullTarget() {
    ...
    if (!_speculating && pull_target - old_pull_target < PULL_QUANTUM && _credit_spec > 0 &&
        (true || _backlog > 0 || !_rtx_queue.empty())) {
        _credit_spec -= _mtu;
        pull_target += _mtu;
    }
            ...
            if (_credit_pull > 0) {
                if (_debug_src) {
                    cout << _flow.str() << " " << "we have " << _credit_pull
                         << " bytes of credit, but nothing to use it on" << " flow " << _flow.str()
                         << endl;
                }
            }
            return;
...
mem_b EqdsSrc::sendNewPacket(const Route& route) {
    if (_debug_src)
        cout << _flow.str() << " " << _nodename << " sendNewPacket highest_sent " << _highest_sent
             << " h*m " << _highest_sent * _mss << " backlog " << _backlog << " unsent " << _unsent
             << " flow " << _flow.str() << endl;
    assert(_unsent > 0);
    ...
    auto*    p  = EqdsDataPacket::newpkt(_flow,
                                     route,
                                     _highest_sent,
                                     full_pkt_size,
                                     ptype,
...
mem_b EqdsSrc::sendRtxPacket(const Route& route) {
    ...
    assert(
        !speculative);  // I don't think this can happen, but remove this assert if we decide it can
    ...
    if (_debug_src)
        cout << _flow.str() << " " << _nodename << " sending rtx pkt " << seq_no << " size "
             << full_pkt_size << " at " << timeAsUs(eventlist().now()) << " cwnd " << _cwnd
...
void EqdsSrc::sendRTS(bool timeout) {
    ...
    if (_debug_src)
        cout << _flow.str() << " " << _nodename << " sendRTS, flow " << _flow.str() << " at "
             << timeAsUs(eventlist().now()) << " epsn " << _highest_sent << " last RTS "
             << timeAsUs(_last_rts) << " in_flight " << _in_flight << " pull_target "
             << _pull_target << " pull " << _pull << endl;
    createSendRecord(_highest_sent, _hdr_size);
    auto* p = EqdsRtsPacket::newpkt(_flow, NULL, _highest_sent, _pull_target, timeout, _dstaddr);
    p->set_dst(_dstaddr);
    uint16_t ev = nextEntropy();
    p->set_pathid(ev);
...
//  EQDS SINK PORT
////////////////////////////////////////////////////////////////
EqdsSinkPort::EqdsSinkPort(EqdsSink& sink, uint32_t port_num) : _sink(sink), _port_num(port_num) {}
...
void EqdsSink::handlePullTarget(EqdsBasePacket::seq_t pt) {
    if (_src->debug())
        cout << " EqdsSink " << _nodename << " src " << _src->nodename() << " handlePullTarget pt "
