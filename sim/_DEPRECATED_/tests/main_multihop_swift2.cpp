...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < 20; i++) {
        ...
        if (i == 3)
            ql = &queueLogger;  // only log queue[3] as that's the bottleneck queue
        if (i == 1)
            ql = &queueLogger2;  // only log queue[3] as that's the bottleneck queue
    ...
    SwiftSinkLoggerSampling sinkLogger =
        SwiftSinkLoggerSampling(timeFromUs((uint32_t)100), eventlist);
