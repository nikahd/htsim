...
int main(int argc, char** argv) {
    ...
    // QueueLoggerSampling qs1 =
    // QueueLoggerSampling(timeFromMs(10),eventlist);logfile.addLogger(qs1);
    //  Build the network
    ...
    STrackSinkLoggerSampling sinkLogger =
        STrackSinkLoggerSampling(timeFromUs((uint32_t)1000), eventlist);
    ...
    for (int i = 0; i < no_of_conns; i++) {
        ...
        strackSrc->setName("STRACK" + ntoa(i));
        logfile.writeName(*strackSrc);

        strackSnk = new STrackSink();
        // strackSnk = new STrackSinkTransfer();
        strackSnk->setName("STRACKSink" + ntoa(i));
        logfile.writeName(*strackSnk);
        ...
        strackSrc->connect(*routeout,
                           *routein,
                           *strackSnk,
                           timeFromMs(i * 5));  // drand()*timeFromMs(1));
