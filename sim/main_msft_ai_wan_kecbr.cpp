...
void executeTraffixMatrix(Logfile& logfile, uint64_t base_inter_rtt, uint64_t base_intra_rtt) {
    ...
    vector<KECbrSrc*>            intra_cbr_srcs;
    vector<KECbrSrc*>            inter_srcs;
    KECbrSrcRestartTimerScanner  cbrSrcRestartScanner(timeFromMs(20), eventlist);
    KECbrSinkRestartTimerScanner cbrSinkRestartScanner(timeFromMs(20), eventlist);
...
int main(int argc, char** argv) {
    ...
    KECbrSrc::setRouteStrategy(sim_params.route_strategy);
    KECbrSink::setRouteStrategy(sim_params.route_strategy);

    KECbrSrc::set_use_erasure_coding(sim_params.use_erasure_coding);
    KECbrSink::set_use_erasure_coding(sim_params.use_erasure_coding);
    KECbrSrc::set_erasure_coding(new ErasureCoding(sim_params.erasure_coding_k,
                                                   sim_params.erasure_coding_m,
                                                   sim_params.erasure_coding_stripe_num,
                                                   sim_params.number_entropies));
    KECbrSink::set_erasure_coding(new ErasureCoding(sim_params.erasure_coding_k,
                                                    sim_params.erasure_coding_m,
                                                    sim_params.erasure_coding_stripe_num,
                                                    sim_params.number_entropies));
