...
void executeTraffixMatrix(Logfile& logfile, uint64_t base_inter_rtt, uint64_t base_intra_rtt) {
    ...
    vector<OneECbrSrc*>            intra_cbr_srcs;
    vector<OneECbrSrc*>            inter_srcs;
    OneECbrSrcRestartTimerScanner  cbrSrcRestartScanner(timeFromMs(20), eventlist);
    OneECbrSinkRestartTimerScanner cbrSinkRestartScanner(timeFromMs(20), eventlist);
    ...
    for (size_t c = 0; c < all_conns->size(); c++) {
        ...
        cbrSrc  = new OneECbrSrc(eventlist,
                                speedFromPktps(sim_params.init_cwnd_ratio * sim_params.cbr_rate),
                                statistics_outfile,
                                Packet::PRIO_LO);  // 70% of 100 Gbps
        cbrSink = new OneECbrSink(eventlist);
...
int main(int argc, char** argv) {
    ...
    if (sim_params.wan_params.n_region > 1) {
        ...
        if (sim_params.use_jitter) {
            for (size_t idx = 0; idx < sim_params.link_params.link_factor_regional[0][1]; idx++) {
                latency_pool.push_back(timeFromUs((uint32_t)(1500 + idx * 333)));
            }
        } else {
            for (size_t idx = 0; idx < sim_params.link_params.link_factor_regional[0][1]; idx++) {
                latency_pool.push_back(timeFromUs((uint32_t)(1500)));
            }
        }
    ...
    OneECbrSrc::setRouteStrategy(sim_params.route_strategy);
    OneECbrSink::setRouteStrategy(sim_params.route_strategy);

    OneECbrSrc::set_use_erasure_coding(sim_params.use_erasure_coding);
    OneECbrSink::set_use_erasure_coding(sim_params.use_erasure_coding);
    OneECbrSrc::set_erasure_coding(new ErasureCoding(sim_params.erasure_coding_k,
                                                     sim_params.erasure_coding_m,
                                                     sim_params.erasure_coding_stripe_num,
                                                     sim_params.number_entropies));
    OneECbrSink::set_erasure_coding(new ErasureCoding(sim_params.erasure_coding_k,
                                                      sim_params.erasure_coding_m,
                                                      sim_params.erasure_coding_stripe_num,
                                                      sim_params.number_entropies));
