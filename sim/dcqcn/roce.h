...
public:
    RoceSrc(RoceLogger*    logger,
            TrafficLogger* pktlogger,
            EventList&     eventlist,
            linkspeed_bps  rate,
            ofstream&      statistics_outfile);

    virtual void connect(Route*          routeout,
                         Route*          routeback,
                         RoceSink&       sink,
    ...
    void setRate(linkspeed_bps r) {
        _bitrate        = r;
        _packet_spacing = (simtime_picosec)((Packet::data_packet_size() + RocePacket::ACKSIZE) *
    ...
    void         ACKTimeout();
    void         ack_timer_hook(simtime_picosec now, simtime_picosec period);

    virtual mem_b queuesize() const { return 0; };

    virtual mem_b maxsize() const { return 0; };

    simtime_picosec _ack_timeout_period  = timeFromUs((uint32_t)60000);  // 3 * RTT
    simtime_picosec _ack_timeout         = timeInf;
    bool            _ack_timeout_pending = false;

    // should really be private, but loggers want to see:
    uint64_t _highest_sent, _old_highest_sent;  // seqno in psn
    ...
    bool get_flow_finished() { return _done; }
    ...
    static uint32_t
        _global_rto_count;  // keep track of the total number of timeouts across all srcs
...
class RoceSink : public EventSource, public PacketSink, public DataReceiver {
    friend class RoceSrc;

public:
    RoceSink(EventList& eventlist, ofstream& statistics_outfile);
    ...
    ModularVector<uint8_t, roceMaxReorder>
...
class RoceSrcACKTimerScanner : public EventSource {
public:
    RoceSrcACKTimerScanner(simtime_picosec scanPeriod, EventList& eventlist);
    void doNextEvent();
    void registerRoce(RoceSrc& rocesrc);
