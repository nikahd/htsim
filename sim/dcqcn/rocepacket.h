...
public:
    ...
    inline static RocePacket* newpkt(PacketFlow&  flow,
                                     ...
                                     uint32_t     destination = UINT32_MAX) {
        RocePacket* p = _packetdb.allocPacket();
        // TODO: check packet database / new / free; print eventlist size; try simulation with no
        // packet loss in WAN;
        // TODO: compare CBR eventlist size vs DCQCN
        p->set_route(flow,
                     route,
                     size + ACKSIZE,
                     seqno + size - 1);  // The NDP sequence number is the first byte of the packet;
...
public:
    ...
    inline static RoceAck* newpkt(PacketFlow&  flow,
                                  const Route& route,
                                  seq_t        ackno,
                                  uint32_t     destination = UINT32_MAX) {
...
public:
    ...
    inline static RoceNack* newpkt(PacketFlow&  flow,
                                   const Route& route,
                                   seq_t        ackno,
                                   uint32_t     destination = UINT32_MAX) {
