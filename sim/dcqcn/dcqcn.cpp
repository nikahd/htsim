...
simtime_picosec DCQCNSink::_cnp_interval =
    timeFromUs(50.0);  // making this the same as Metric-ACK in EC_CC
simtime_picosec DCQCNSrc::_cc_update_period = timeFromUs(15000.0);
...
void DCQCNSrc::processCNP(const CNPPacket& cnp) {
    ...
    // cout << "At " << timeAsUs(eventlist().now()) << " " << _RC
    //      << " CNP received, reduced rate; target rate is " << _RT << " node " << nodename()
    //      << " alpha is " << _alpha;
...
void DCQCNSrc::increaseRate() {
    // cout << "inside increaseRate() _RC: " << _RC << " _link: " << _link << endl;
    ...
    cout << "At " << timeAsUs(eventlist().now()) << " " << _RC << " increase target rate is " << _RT
         << " node " << nodename() << " packet spacing is " << timeAsUs(_packet_spacing)
         << " _T is " << _T << " _BC is " << _BC << " byte counter is " << _byte_counter
         << " alpha is " << _alpha << endl;
}
...
void DCQCNSrc::doNextEvent() {
    ...
    // if ((_highest_sent - _old_highest_sent) > 0)
    //     cout << "Adding " << (_highest_sent - _old_highest_sent) * _mss << " to sent bytes "
    //          << _byte_counter << " " << _highest_sent << " flow_id: " << _name << endl;
...
DCQCNSink::DCQCNSink(EventList& eventlist, ofstream& statistics_outfile) : RoceSink(eventlist, statistics_outfile) {
...
void DCQCNSink::receivePacket(Packet& pkt) {
    // cout << "DCQCNSink received packet at " << timeAsUs(eventlist().now())
        //  << " pkt type: " << pkt.type() << " flow_id: " << _src->_name << endl;

    bool ecn_marked = ((pkt.flags() & ECN_CE) != 0);
    RoceSink::receivePacket(pkt);

    if (ecn_marked) {
        cout << "DCQCNSink received ECN marked packet at " << timeAsUs(eventlist().now())
             << " generating CNP" << endl;
