...
struct simulation_parms {
    // 1EC parameters
    uint32_t bitmap_size_per_path  = 2048;
    uint32_t cbr_rate              = 3004806;  // in pktps, 3004806 is 100% of 100 Gbps
    ...
    // CC parameters
    bool   use_replace_path                          = true;
    bool   apply_mimd                                = true;
...
void executeTraffixMatrix(Logfile& logfile, uint64_t base_inter_rtt, uint64_t base_intra_rtt) {
    ...
    vector<OneECCCSrc*>              intra_cbr_srcs;
    vector<OneECCCSrc*>              inter_srcs;
    OneECCCSrcRestartTimerScanner    cbrSrcRestartScanner(timeFromMs(40), eventlist);
    OneECCCSinkRestartTimerScanner   cbrSinkRestartScanner(timeFromMs(40), eventlist);
    OneECCCSinkMetricACKTimerScanner cbrSinkMetricACKScanner(timeFromUs((uint32_t)50), eventlist);
    ...
    for (size_t c = 0; c < all_conns->size(); c++) {
        ...
        cbrSrc = new OneECCCSrc(
            eventlist, speedFromPktps(sim_params.cbr_rate), statistics_outfile, Packet::PRIO_LO);
        cbrSink = new OneECCCSink(eventlist, statistics_outfile);

        cbrSrc->updateParams(sim_params.max_rtt,
                             sim_params.link_params.intra_dc_link_speed,
                             sim_params.init_cwnd_ratio,
                             sim_params.contract_scaling,
                             sim_params.queue_params.OWR_queuesize);
        cbrSrc->set_jittery_path_replace_threshold_percentage(
            sim_params.jittery_path_replace_threshold_percentage);
        cbrSrc->set_use_replace_path(sim_params.use_replace_path);
        cbrSrc->set_apply_mimd(sim_params.apply_mimd);

        cbrSrc->set_hash_salt(random());
        cbrSink->set_hash_salt(random());

        cbrSrcRestartScanner.registerCbr(*cbrSrc);
        cbrSinkRestartScanner.registerCbr(*cbrSink);
        cbrSinkMetricACKScanner.registerCbr(*cbrSink);
        ...
        logfile.writeName(static_cast<DataReceiver&>(*cbrSink));

        cbrSink->set_src(src);
        cbrSink->set_lossy_threshold(sim_params.lossy_threshold);
        cbrSink->set_recoverable_threshold(sim_params.recoverable_threshold);
        cbrSink->set_use_bitmap_full_condition_lossy_skip(
            sim_params.use_bitmap_full_condition_lossy_skip);
        cbrSink->set_use_as_fast_as_possible_recoverable_skip(
            sim_params.use_as_fast_as_possible_recoverable_skip);
        cbrSink->set_use_bitmap_full_percentage(sim_params.bitmap_full_percent);
        cbrSink->set_use_full_skip(sim_params.use_full_skip);
        cbrSink->set_loss_path_replace_threshold(sim_params.loss_path_replace_threshold_percentage);
        ...
        switch (sim_params.route_strategy) {
            ...
            case REACTIVE_ECN: {
                ...
                cbrSrc->set_paths(sim_params.number_entropies);
                cbrSink->set_paths(sim_params.number_entropies);
    ...
    for (auto iter = inter_srcs.begin(); iter != inter_srcs.end(); ++iter) {
        OneECCCSrc* cbrsrc = *iter;
        finished_flow_number += int(cbrsrc->_flow_finished);
    }
    ...
    for (std::size_t i = 0; i < intra_cbr_srcs.size(); ++i) {
        delete intra_cbr_srcs[i];
    }
}
...
int main(int argc, char** argv) {
    ...
    if (sim_params.wan_params.n_region > 1) {
        ...
        sim_params.max_rtt = (max_latency + timeFromUs((uint32_t)303)) * 2;
    } else {
        sim_params.owr_owr_hop_latency.push_back(timeFromUs((uint32_t)14000));
    ...
    OneECCCSrc::setRouteStrategy(sim_params.route_strategy);
    OneECCCSink::setRouteStrategy(sim_params.route_strategy);

    OneECCCSrc::set_use_erasure_coding(sim_params.use_erasure_coding);
    OneECCCSink::set_use_erasure_coding(sim_params.use_erasure_coding);
    OneECCCSrc::set_erasure_coding(new ErasureCoding(sim_params.erasure_coding_k,
                                                     sim_params.erasure_coding_m,
                                                     sim_params.erasure_coding_stripe_num,
                                                     sim_params.number_entropies));
    OneECCCSink::set_erasure_coding(new ErasureCoding(sim_params.erasure_coding_k,
                                                      sim_params.erasure_coding_m,
                                                      sim_params.erasure_coding_stripe_num,
                                                      sim_params.number_entropies));
...
void parseCommandLine(int argc, char** argv) {
    ...
    while (i < argc) {
        if (!strcmp(argv[i], "-o")) {
            ...
            i++;
        } else if (!strcmp(argv[i], "-bitmap-size")) {
            cout << "Bitmap Size Per Path: " << argv[i + 1] << endl;
            sim_params.bitmap_size_per_path = atoi(argv[i + 1]);
            i++;
        } else if (!strcmp(argv[i], "-use-replace-path")) {
            cout << "Use replace path: " << argv[i + 1] << endl;
            sim_params.use_replace_path = atoi(argv[i + 1]);
            i++;
        } else if (!strcmp(argv[i], "-apply-mimd")) {
            cout << "Apply MIMD: " << argv[i + 1] << endl;
            sim_params.apply_mimd = atoi(argv[i + 1]);
            i++;
        } else if (!strcmp(argv[i], "-contract-scaling")) {
            cout << "Contract Scaling: " << argv[i + 1] << endl;
            sim_params.contract_scaling = atof(argv[i + 1]);
            i++;
