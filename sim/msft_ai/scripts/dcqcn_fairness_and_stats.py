...
# --------------------------
# Helpers to derive filepaths
# --------------------------

def derive_statistics_path(sim_out_path: str) -> str:
    """
    Convert:
      msft_ai_wan_single_dcqcn/output_one_one_4_200MB_linkdown0_droprate0_usejitter0_exp1.txt
    -> msft_ai_wan_single_dcqcn/statistics_one_one_4_200MB_linkdown0_droprate0_usejitter0_exp1.txt
    """
    basedir = os.path.dirname(sim_out_path)
    base = os.path.basename(sim_out_path)
    stats = base.replace("output_", "statistics_")
    return os.path.join(basedir, stats)

def derive_plot_path(stat_path: str) -> str:
    """
    Creates a fairness plot path like:
      <folder>/option_plots/fairness_<matrix>_link<link>_usejitter<j>_droprate<d>_initcwnd0.7.png
    where these tokens are parsed from 'statistics_<matrix>_linkdownX_droprateY_usejitterZ_expK.txt'
    """
    ...
    # Expect: statistics_<matrix>_linkdownX_droprateY_usejitterZ_expK.txt
...
# --------------------------
# Part A: Parse statistics_*.txt and draw sending-rate plot
# --------------------------

def parse_sending_rates(stat_path: str):
    """
    stats file lines look like:
      Flow <id> time: <us> sending_rate: <bps>
    or
      Flow DCQCN0 time: <us> sending_rate: <bps>
    Returns dict: flow_id(str) -> list[(time_us(float), rate_bps(float))]
    """
    ...
    with open(stat_path, "r") as f:
        for raw in f:
            ...
            if not line or "Flow" not in line or "sending_rate" not in line:
                continue
            # robust parse
            # Accept either: "Flow <id> time: X sending_rate: Y"
            # or:           "Flow DCQCN0 time: X sending_rate: Y"
            try:
                # Flow token is always first, next token is id
                # then "time:", value, "sending_rate:", value
                parts = line.split()
                flow_id = parts[1]  # keep as string "DCQCN0" or "1000000001"
                # find keywords
                t_idx = parts.index("time:") + 1
                r_idx = parts.index("sending_rate:") + 1
                t_us = float(parts[t_idx])
                r_bps = float(parts[r_idx])
                sending_rate_dict[flow_id].append((t_us, r_bps))
            except Exception:
                # quiet skip if format mismatch
                continue
    return sending_rate_dict

def _color_for(flow_id: str, palette):
    """
    Stable but simple color pick; supports numeric or 'DCQCNX'
    """
    try:
        if flow_id.startswith("DCQCN"):
            idx = int(flow_id[5:])
        else:
            idx = int(flow_id) % 1000  # try numeric
    except Exception:
...
# --------------------------
# Part B: Analyze traces (paths + ECN/CNP + FCT + score)
# --------------------------
...
def main():
    if len(sys.argv) < 4:
        print("Usage:\n  python3 dcqcn_fairness_and_stats.py <sim_out.txt> <trace_packets.csv> <paths_file>")
        sys.exit(1)
    ...
    # A) derive & parse statistics_*.txt and draw plot
    stat_path = derive_statistics_path(sim_out_file)
    sending_rate_dict = parse_sending_rates(stat_path)
    plot_path = derive_plot_path(stat_path)
    # build a simple title using tokens from plot_path name
    title = f"Sending rates for {os.path.basename(plot_path).replace('fairness_','').replace('.png','')}"
    draw_fairness_plot(sending_rate_dict, plot_path, title)

    # B) flow table + FCT + score
    print_and_write_summary(trace_csv, paths_file)
