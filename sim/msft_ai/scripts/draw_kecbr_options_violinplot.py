                            ...
                            for droprate in drop_rate:
                                ...
                                for bitmapsize in bitmap_size:
                                    ...
                                    for exp in experiments:
                                        sim_file_name = f"{folder_name}/statistics_{matrix[:-3]}_linkdown{link_down}_droprate{droprate}_usejitter{use_jitter}_initcwnd{init_cwnd}_bitmapsize{bitmapsize}_recoverable{recoverable}_isfullskip{isfullskip}_fastrecoverable{fast_rec}_bitmapfulllossy1_fullpercent{percent}_exp{exp}.txt"
                                        ...
                                        with open(sim_file_name, "r") as f:
                                            ...
                                            for line in lines:
                                                if "flow_completion_time" in line:
                                                    ...
                                                    max_size_bitmaps = [0] * 8
                                                    max_size_bitmaps[0] = int(parts[20].strip(','))
                                                    max_size_bitmaps[1] = int(parts[22].strip(','))
                                                    max_size_bitmaps[2] = int(parts[24].strip(','))
                                                    max_size_bitmaps[3] = int(parts[26].strip(','))
                                                    max_size_bitmaps[4] = int(parts[28].strip(','))
                                                    max_size_bitmaps[5] = int(parts[30].strip(','))
                                                    max_size_bitmaps[6] = int(parts[32].strip(','))
                                                    max_size_bitmaps[7] = int(parts[34].strip(','))
                                                    ...
                                                    logging_statistics["exp"+str(exp) + cur_flow_id] = {
                                                        ...
                                                        "max_size_bitmaps": max(max_size_bitmaps),
                                    ...
                                    total_statistics[percent][fast_rec][isfullskip][recoverable][init_cwnd][matrix][link_down][droprate][bitmapsize] = logging_statistics
...
plot_statistics = ["flow_completion_time",
                   ...
                   "num_bitmap_underflow_drops", "num_trivial_skips", "num_recoverable_skips", "num_lossy_skips", "max_size_bitmaps"]
                ...
                for bitmapsize in bitmap_size:
                    for percent in full_percent:
                        for rec in recoverable_threshold:
                            group_combos.append((bitmapsize, percent, rec))
                            n_groups += 1

                    y_values = []
                    for group_idx, (bitmapsize, percent, rec) in enumerate(group_combos):
                    
                        pre_aggregated_statistics = []

                        stats_dict = total_statistics[percent][fast_rec][isfullskip][rec][init_cwnd][matrix][link_down][droprate][bitmapsize]
                    ...
                    plt.title(f"{get_ylabel(plots)} vs Initial CWND Ratio for {matrix} with Bitmap {1024}")
                    plt.xticks(np.arange(1, len(group_combos) + 1),
                            [f"Bitmap{g[0]},\nOcc{g[1]},\nRec{g[2]}" for g in group_combos],
                            rotation=0, ha='right')
