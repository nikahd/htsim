...
flow_numbers = [1, 4, 2, 4, 1, 2, 4, 8, 16, 32, 64, 128, 256]  # Number of flows for each connection matrix, should be aligned with connection_matrices array
                                        ...
                                        for jitter_replace in jitter_path_replace_threshold:
                                            ...
                                            for exp in experiments:
                                                ...
                                                with open(sim_file_name, "r") as f:
                                                    ...
                                                    for line in lines:
                                                        if "cwnd" in line and "Flow" in line and "time" in line:
                                                            parts = line.split()
                                                            flow_id = parts[1]
                                                            timestamp = float(parts[3])
                                                            cwnd_in_packets = float(parts[5])
...
def plot_sending_rate(sending_rate_dict):
    for flow_id, rates in sending_rate_dict.items():
        ...
        sending_rates = [rate for rate in sending_rates]  
...
for matrix in connection_matrices:
    for link_down in is_link_down:
        for droprate in drop_rate:
            for bitmapsize in bitmap_size:
                figure_file_name = f"{folder_name}/option_plots/fairness_{matrix}_link{link_down}_usejitter{use_jitter}_droprate{droprate}_mimd{apply_mimd}_initcwnd{init_cwnd}_pfc{enable_pfc}_cwnd.png"
                ...
                for group_idx, (percent, rec, loss_replace, jitter_replace) in enumerate(group_combos):
                    plot_sending_rate(total_statistics[percent][fast_rec][isfullskip][rec][init_cwnd][matrix][link_down][droprate][bitmapsize][loss_replace][jitter_replace])
