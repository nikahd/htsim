...
use_jitter = 0
apply_mimd = 1
contract_scalings=[1, 2, 4, 6, 8, 10]
                                ...
                                for bitmapsize in bitmap_size:
                                    ...
                                    for loss_replace in loss_path_replace_threshold:
                                        ...
                                        for jitter_replace in jitter_path_replace_threshold:
                                            ...
                                            for contract_scaling in contract_scalings:
                                                total_statistics[percent][fast_rec][isfullskip][recoverable][init_cwnd][matrix][link_down][droprate][bitmapsize][loss_replace][jitter_replace][contract_scaling] = defaultdict(dict)
                                                logging_statistics = defaultdict(dict)
                                                for exp in experiments:
                                                    sim_file_name = f"{folder_name}/statistics_{matrix[:-3]}_linkdown{link_down}_droprate{droprate}_usejitter{use_jitter}_initcwnd{init_cwnd}_bitmapsize{bitmapsize}_recoverable{recoverable}_isfullskip{isfullskip}_fastrecoverable{fast_rec}_bitmapfulllossy1_fullpercent{percent}_lossreplace{loss_replace}_jitterreplace{jitter_replace}_mimd{apply_mimd}_contract{contract_scaling}_exp{exp}.txt"
                                                    ...
                                                    with open(sim_file_name, "r") as f:
                                                        ...
                                                        for line in lines:
                                                            if "cwnd" in line and "Flow" in line and "time" in line and "com" not in line:
                                                                parts = line.split()
                                                                flow_id = parts[1]
                                                                timestamp = float(parts[3])
                                                                cwnd_in_packets = float(parts[-1])
                                                                if flow_id not in cwnd_dict:
                                                                    cwnd_dict[flow_id] = []
                                                                cwnd_dict[flow_id].append((timestamp, cwnd_in_packets))
                                                        ...
                                                        total_statistics[percent][fast_rec][isfullskip][recoverable][init_cwnd][matrix][link_down][droprate][bitmapsize][loss_replace][jitter_replace][contract_scaling] = cwnd_dict


def plot_sending_rate(sending_rate_dict):
    for flow_id, rates in sending_rate_dict.items():
        timestamps, sending_rates = zip(*rates)
        sending_rates = [rate for rate in sending_rates]  # Convert to packets
...
for matrix in connection_matrices:
    for link_down in is_link_down:
        for droprate in drop_rate:
            for bitmapsize in bitmap_size:
                for contract_scaling in contract_scalings:
                    figure_file_name = f"{folder_name}/option_plots/fairness_{matrix}_link{link_down}_usejitter{use_jitter}_droprate{droprate}_mimd{apply_mimd}_contract{contract_scaling}_initcwnd0.7.png"
                    ...
                    for group_idx, (percent, rec, loss_replace, jitter_replace) in enumerate(group_combos):
                        plot_sending_rate(total_statistics[percent][fast_rec][isfullskip][rec][init_cwnd][matrix][link_down][droprate][bitmapsize][loss_replace][jitter_replace][contract_scaling])
                    ...
                    plt.title(f"CWND for {matrix} (Link Down: {link_down}, Drop Rate: {droprate})")
                    plt.xlabel("Time (us)")
