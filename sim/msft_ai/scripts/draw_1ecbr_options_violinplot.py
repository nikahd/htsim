                                    ...
                                    for loss_replace in loss_path_replace_threshold:
                                        ...
                                        for jitter_replace in jitter_path_replace_threshold:
                                            ...
                                            for exp in experiments:
                                                sim_file_name = f"{folder_name}/statistics_{matrix[:-3]}_linkdown{link_down}_droprate{droprate}_usejitter{use_jitter}_initcwnd{init_cwnd}_bitmapsize{bitmapsize}_recoverable{recoverable}_isfullskip{isfullskip}_fastrecoverable{fast_rec}_bitmapfulllossy1_fullpercent{percent}_lossreplace{loss_replace}_jitterreplace{jitter_replace}_exp{exp}.txt"
                                                if jitter_replace == 0 and loss_replace == 0:
                ...
                for bitmapsize in bitmap_size:
                    for percent in full_percent:
                        for rec in recoverable_threshold:
                            for loss_replace in loss_path_replace_threshold:
                                for jitter_replace in jitter_path_replace_threshold:
                                    group_combos.append((bitmapsize, percent, rec, loss_replace, jitter_replace))
                                    n_groups += 1

                    y_values = []
                    for group_idx, (bitmapsize, percent, rec, loss_replace, jitter_replace) in enumerate(group_combos):
                    ...
                    plt.xticks(np.arange(1, len(group_combos) + 1),
                            [f"Bitmap{g[0]},\nOcc{g[1]},\nRec{g[2]},\nLoss\nReplace{g[3]},\nJitter\nReplace{g[4]}" for g in group_combos],
                            rotation=0, ha='right')
