...
class KECCCSink;
class KECSmarttPacer;

#define timeInf 0

// TODO: rename CBR to ECCbrBaseline

class KECCCSrc : public EventSource, public PacketSink {
    friend class KECCCSink;

public:
    KECCCSrc(EventList&          eventlist,
             linkspeed_bps       rate,
             ofstream&           statistics_outfile,
             Packet::PktPriority priority = Packet::PRIO_NONE);

    void connect(Route* routeout, Route* routeback, KECCCSink& sink, simtime_picosec startTime);
    ...
    void send_packet_ec(uint32_t msn,
    ...
    static void setECAlgorithm(const string& algo) { _ec_algorithm = algo; }
    ...
    void adjust_window(uint64_t                 num_bytes_acked,
                       simtime_picosec          ts,
                       vector<simtime_picosec>& per_path_rtt,
    ...
    void set_jittery_path_replace_threshold_percentage(double percent) {
        if (percent < 0.0 || percent > 1.0) {
            throw std::runtime_error(
                "KECCCSrc::set_jittery_path_replace_threshold_percentage: "
    ...
    bool            apply_mimd         = false;  // congestion control type, default is mimd
...
public:
    ...
    logging_statistics statistics;  // per-path statistics

    void update_bitmap_statistics(size_t bitmap_idx);

    KECCCSink(EventList& eventlist);
    ...
    void set_use_recoverable_skip(bool use) { _use_recoverable_skip = use; }

    void set_src(uint32_t s) { _srcaddr = s; }

    void connect(KECCCSrc& src, const Route* route);
    ...
    void update_mbl(CbrPacket& pkt, uint32_t per_path_psn, uint32_t last_per_path_psn);

    inline bool     is_bitmap_full();
    inline uint32_t count_consecutive_ones(const bitset<ErasureCoding::BITMAP_SIZE_KEC>& bitmap);

    inline uint32_t count_ones(const bitset<ErasureCoding::BITMAP_SIZE_KEC>& bitmap, uint32_t size);

    inline bool is_num_consequtive_ones(const bitset<ErasureCoding::BITMAP_SIZE_KEC>& bitmap,
                                        uint32_t                                      k);

    inline size_t find_highest_bit(const bitset<ErasureCoding::BITMAP_SIZE_KEC>& bitmap);

    uint32_t check_stripe_bitmap_update(uint32_t bitmap_idx);

    void shift_stripe(uint32_t bitmap_idx, uint32_t shift_bits);
    bool check_flow_active();

    tuple<bool, uint32_t> trivial_skip_condition(
        const vector<bitset<ErasureCoding::BITMAP_SIZE_KEC>>& bitmaps, uint32_t k);
    tuple<bool, uint32_t> recoverable_skip_condition(
        const vector<bitset<ErasureCoding::BITMAP_SIZE_KEC>>& bitmaps, uint32_t k);

    tuple<uint32_t, bool> check_pkt(uint32_t per_path_psn,
                                    uint32_t bitmap_idx,
                                    bool     _is_skip_bitmap_one_chunk,
                                    uint32_t last_per_path_psn,
                                    uint32_t last_virtual_per_path_psn);
    tuple<uint32_t, bool> check_pkt_full_skip(uint32_t per_path_psn,
                                              uint32_t bitmap_idx,
                                              bool     _is_skip_bitmap_one_chunk,
                                              uint32_t last_per_path_psn,
                                              uint32_t last_virtual_per_path_psn);
    uint32_t              check_lossy_skip(bool do_full_skip = false);
    uint32_t              check_trivial_skip(bool _is_skip_bitmap_one_chunk);
    uint32_t              check_recoverable_skip(uint32_t per_path_psn,
                                                 bool     _is_skip_bitmap_one_chunk,
                                                 bool     do_full_skip = false);
    uint32_t              check_virtual_epsns(uint32_t current_virtual_epsn);
    // uint32_t check_full_skip(uint32_t per_path_psn, uint32_t bitmap_idx);
    bool is_full_skip_condition();
    bool is_lossy_skip_condition();
    bool is_recoverable_skip_condition();

    void update_global_epsn();

    void complete_mbl(Packet& pkt, simtime_picosec ts, uint32_t epsn);
    ...
    void set_loss_path_replace_threshold(double percent) {
        ...
        loss_path_replace_threshold = percent * ErasureCoding::BITMAP_SIZE_KEC;
    }
    ...
    uint32_t global_min_epsn      = 0;  // minimum epsn across per-path bitmaps
    uint32_t global_max_psn       = 0;  // defined as global_max_epsn - globa_min_epsn
    uint32_t current_virtual_epsn = 0;
...
private:
    ...
    // retransmission logic with per-path bitmap and message boundary list
    vector<uint32_t> per_path_epsns;  // per-path epsn for each bitmap
    vector<uint32_t> per_path_max_psns;
    uint32_t         full_skip_threshold        = 50;  // in terms of stripes
    uint32_t         lossy_skip_threshold       = 28;  // in terms of stripes; number of paths
    uint32_t         recoverable_skip_threshold = 20;  // in terms of stripes;

    uint32_t         mbl_base_msn = 0;       // base msn for the message boundary list at head
    vector<uint32_t> message_boundary_list;  // shared mbl for all paths, updated with per-path epsn
    int              mbl_head = 0;           // head of the message boundary list
    int              mbl_tail = 0;           // tail of the message boundary list
    int              mbl_size = 0;
    vector<bitset<ErasureCoding::BITMAP_SIZE_KEC>> per_path_bitmaps;     // per-path bitmap
    ...
    uint32_t                loss_path_replace_threshold = 0.5 * ErasureCoding::BITMAP_SIZE_KEC;

    // configuration
    bool _is_skip_bitmap_one_chunk = false;  // whether the skip bitmap is one chunk or as many
                                             // chunks as possible in receivePacket()
    bool _use_full_skip        = false;
    bool _use_recoverable_skip = true;
...
class KECCCSrcRestartTimerScanner : public EventSource {
public:
    KECCCSrcRestartTimerScanner(simtime_picosec scanPeriod, EventList& eventlist);
    void doNextEvent();
    void registerCbr(KECCCSrc& cbrsrc);

private:
    simtime_picosec         _scanPeriod;
    typedef list<KECCCSrc*> cbrs_t;
    cbrs_t                  _cbrs;
};

class KECCCSinkRestartTimerScanner : public EventSource {
public:
    KECCCSinkRestartTimerScanner(simtime_picosec scanPeriod, EventList& eventlist);
    void doNextEvent();
    void registerCbr(KECCCSink& cbrsink);

private:
    simtime_picosec          _scanPeriod;
    typedef list<KECCCSink*> cbrs_t;
    cbrs_t                   _cbrs;
};

class KECCCSinkMetricACKTimerScanner : public EventSource {
public:
    KECCCSinkMetricACKTimerScanner(simtime_picosec scanPeriod, EventList& eventlist);
    void doNextEvent();
    void registerCbr(KECCCSink& cbrsink);
