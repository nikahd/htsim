...
class AIRegionMsoftTest : public ::testing::Test {
protected:
    // === Simulation params
    uint32_t RTT = 400;  // this is per link delay in ns; identical RTT microseconds
                         // = 0.02 ms
    linkspeed_bps   intra_dc_linkspeed = speedFromMbps((double)HOST_NIC);
    linkspeed_bps   inter_dc_linkspeed = speedFromMbps((double)400);
    ...
    struct AI_wan_topology_params AI_region_topo_params = {
    ...
    const char*       tm_file     = "../sim/msft_ai/testdata/1_routing_region.cm";
    ...
    virtual void SetUp() {
        ...
        topo_AI = new AIRegionMsoft(region_idx,
                                    AI_region_topo_params,
                                    intra_dc_linkspeed,
                                    inter_dc_linkspeed,
                                    queue_params,
        ...
        no_of_nodes = AI_region_topo_params.get_no_of_nodes_in_WAN();
    ...
    vector<AISwitch::node_type> get_switch_type_path(AIRegionMsoft* topo_AI, int src, int dest);
};

vector<AISwitch::node_type> AIRegionMsoftTest::get_switch_type_path(AIRegionMsoft* topo_AI,
                                                                    int            src,
                                                                    int            dest) {
    return get_region_switch_type_path(topo_AI, src, dest);
}

map<uint64_t, int64_t> AIRegionMsoftTest::generate_switch_counter_answers(
    vector<connection*>* all_conns, AIRegionMsoft* topo_AI, struct AI_wan_topology_params params) {
    auto switch_counters = init_region_switch_counters(topo_AI, params);
    ...
    for (size_t c = 0; c < all_conns->size(); c++) {
        ...
        auto [cur_topo_idx, cur_dc]   = get_AI_region_topology_idx(topo_AI, src);
        auto [dest_topo_idx, dest_dc] = get_AI_region_topology_idx(topo_AI, dest);

        update_region_switch_counters(switch_path,
                                      this_flow_id,
                                      cur_topo_idx,
                                      dest_topo_idx,
                                      cur_dc,
...
TEST_F(AIRegionMsoftTest, RoutingTest) {
    ...
    for (size_t c = 0; c < all_conns->size(); c++) {
        ...
        switch (route_strategy) {
            ...
            case REACTIVE_ECN: {
                ...
                AI_region_create_route(
                    topo_AI, src, cbrSrc->_hash_salt, srctotor, cbrSrc->flow_id());
                AI_region_create_route(
                ...
                AI_region_register_host(topo_AI, src, cbrSrc, cbrSrc->flow_id());
                AI_region_register_host(topo_AI, dest, cbrSink, cbrSrc->flow_id());
    ...
    auto ground_truth_counters =
        generate_switch_counter_answers(all_conns, topo_AI, AI_region_topo_params);
