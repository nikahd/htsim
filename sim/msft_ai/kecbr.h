...
class KECbrSrc : public EventSource, public PacketSink {
    friend class KECbrSink;

public:
    KECbrSrc(EventList&          eventlist,
             linkspeed_bps       rate,
             ofstream&           statistics_outfile,
             Packet::PktPriority priority = Packet::PRIO_NONE);

    void connect(Route* routeout, Route* routeback, KECbrSink& sink, simtime_picosec startTime);
...
class KECbrSink : public EventSource, public PacketSink, public DataReceiver {
public:
    ...
    void connect(KECbrSrc& src, const Route* route);
...
class KECbrSrcRestartTimerScanner : public EventSource {
public:
    KECbrSrcRestartTimerScanner(simtime_picosec scanPeriod, EventList& eventlist);
    void doNextEvent();
    void registerCbr(KECbrSrc& cbrsrc);

private:
    simtime_picosec         _scanPeriod;
    typedef list<KECbrSrc*> cbrs_t;
    cbrs_t                  _cbrs;
};

class KECbrSinkRestartTimerScanner : public EventSource {
public:
    KECbrSinkRestartTimerScanner(simtime_picosec scanPeriod, EventList& eventlist);
    void doNextEvent();
    void registerCbr(KECbrSink& cbrsink);

private:
    simtime_picosec          _scanPeriod;
    typedef list<KECbrSink*> cbrs_t;
    cbrs_t                   _cbrs;
};
