...
# CONN_MATRICES=("one_one_32_200MB.cm")
IS_LINK_DOWN=(0)
drop_rates=("0")
use_jitter=0
                                        ...
                                        for loss_replace_percent in "${loss_path_replace_threshold[@]}"; do
                                            for jittery_path_replace in "${jittery_path_replace_threshold[@]}"; do
                                                for ((j=0; j<${#IS_LINK_DOWN[@]}; j++)); do
                                                    ...
                                                    for contract_scaling in "${contract_scalings[@]}"; do
                                                        if [ "$apply_mimd" -eq 0 ]; then
                                                            contract_scaling=1
                                                        else
                                                            contract_scaling=$contract_scaling
                                                        fi
...
-apply-mimd ${apply_mimd} -contract-scaling ${contract_scaling}"
