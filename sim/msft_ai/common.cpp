...
// TODO(zeying): move these global variables into a sim_config struct
...
uint64_t BASE_RTT_MODERN =
    (HOPS * LINK_DELAY_MODERN) + (PKT_SIZE_MODERN * 8 / LINK_SPEED_MODERN * HOPS) +
...
uint64_t MAX_CWD_MODERN_NDP =
    BDP_MODERN_NDP * 3;  // BDP * 3. Based on their choice to use 23 cwd with 9000 pacekts
...
uint64_t BASE_RTT_OLD = (HOPS * LINK_DELAY_OLD) + (PKT_SIZE_OLD * 8 / LINK_SPEED_OLD * HOPS) +
                        (HOPS * LINK_DELAY_OLD) +
...
bool            LCP_USE_DIFFERENT_QUEUES =
    false;  // Whether to use different queues for inter- and intra-DC flows.
bool LCP_APPLY_AI_PER_EPOCH = false;

// global and shared between inter- and intra- lcp
uint32_t LCP_FAST_INCREASE_THRESHOLD_INTRA =
    3;  // Number of consecutive good epochs to trigger fast increase.
uint32_t LCP_FAST_INCREASE_THRESHOLD_INTER =
    3;                          // Number of consecutive good epochs to trigger fast increase.
