...
void executeTraffixMatrix(Logfile& logfile, uint64_t base_inter_rtt, uint64_t base_intra_rtt) {
    ...
    vector<KECCCSrc*>              intra_cbr_srcs;
    vector<KECCCSrc*>              inter_srcs;
    KECCCSrcRestartTimerScanner    cbrSrcRestartScanner(timeFromMs(20), eventlist);
    KECCCSinkRestartTimerScanner   cbrSinkRestartScanner(timeFromMs(20), eventlist);
    KECCCSinkMetricACKTimerScanner cbrSinkMetricACKScanner(timeFromUs((uint32_t)50), eventlist);
    ...
    for (size_t c = 0; c < all_conns->size(); c++) {
        ...
        cbrSrc  = new KECCCSrc(eventlist,
                              speedFromPktps(sim_params.cbr_rate),
                              statistics_outfile,
                              Packet::PRIO_LO);  // 70% of 100 Gbps
        cbrSink = new KECCCSink(eventlist);
...
int main(int argc, char** argv) {
    ...
    if (sim_params.wan_params.n_region > 1) {
        ...
        if (sim_params.use_jitter) {
            for (size_t idx = 0; idx < sim_params.link_params.link_factor_regional[0][1]; idx++) {
                latency_pool.push_back(timeFromUs((uint32_t)(1000 + idx * 333)));
            }
        } else {
            for (size_t idx = 0; idx < sim_params.link_params.link_factor_regional[0][1]; idx++) {
                latency_pool.push_back(timeFromUs((uint32_t)(1000)));
            }
        }
    ...
    KECCCSrc::setRouteStrategy(sim_params.route_strategy);
    KECCCSink::setRouteStrategy(sim_params.route_strategy);

    KECCCSrc::set_use_erasure_coding(sim_params.use_erasure_coding);
    KECCCSink::set_use_erasure_coding(sim_params.use_erasure_coding);
    KECCCSrc::set_erasure_coding(new ErasureCoding(sim_params.erasure_coding_k,
                                                   sim_params.erasure_coding_m,
                                                   sim_params.erasure_coding_stripe_num,
                                                   sim_params.number_entropies));
    KECCCSink::set_erasure_coding(new ErasureCoding(sim_params.erasure_coding_k,
                                                    sim_params.erasure_coding_m,
                                                    sim_params.erasure_coding_stripe_num,
                                                    sim_params.number_entropies));
