...
def plot_total_rtx(data, show_legend, save_dir):
    """
    ...
    if show_legend:
        plt.xlabel("Workload")
        plt.ylabel(f"Total RTX Counts")
        # plt.title("Average Values for Workloads and Setups")
        plt.legend(loc='lower left', bbox_to_anchor=(1, 0))
    plt.ylim([0, 0.12])
    plt.tight_layout()
    plt.savefig(os.path.join(save_dir, f'total_rtx_legend_{"on" if show_legend else "off"}.png'), bbox_inches='tight', pad_inches=0.01, dpi = 500)
    plt.close()
...
if __name__ == "__main__":
    ...
    total_rtx = output['total_rtx']

    total_rtx_data = {}
    for idx in range(len(output['dirpath'])):
        # Read the .npy file
        dirpath = output['dirpath'][idx]
        total_rtx = output['total_rtx'][idx]
        # Extract workload and setup
        workload, setup = extract_workload_and_setup(dirpath)
        # Store data
        try:
            total_rtx_data[workload][setup] = total_rtx
        except:
            total_rtx_data[workload] = {}
            total_rtx_data[workload][setup] = total_rtx

    # plot avg
    plot_total_rtx(total_rtx_data, show_legend=True, save_dir = input_dir)
    plot_total_rtx(total_rtx_data, show_legend=False, save_dir = input_dir)
