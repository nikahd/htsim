...
base_outdir="../outputs_loss_detection_time"
  ...
  for queue_size in "${queue_sizes[@]}"; do
    for drop_prob in "${random_drop_probs[@]}"; do
      ...
      python run_experiment.py -p 32 -o $outdir -t pfld -n 128 \
        ...
        --switch-random-drop-prob $drop_prob --pfld-exp-set 1
      ...
      # plot
      python plot_loss_detection_time.py -i $outdir
      # python plot_loss_detection_time_motivation.py -i $outdir

      # queue size, link overhead
      # python plot_experiment.py -i $outdir/incast16 --stat-vs-msg-size
      # python plot_experiment.py -i $outdir/incast32 --stat-vs-msg-size
